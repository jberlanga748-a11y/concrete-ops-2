...
test("deriveEnterpriseTrustReadinessState summarizes owner trust controls without overclaiming", () => {
  ...
  assert.equal(state.nextActions.some((action) => /SOC 2/i.test(action)), true);
});
...
test("deriveEnterpriseTrustReadinessState fails soft for limited roles or empty audit history", () => {
  ...
  assert.equal(state.nextActions.some((action) => /audit history evidence/i.test(action)), true);
});

test("buildEnterpriseTrustReviewPacket creates a copy-only pilot trust packet without compliance claims", () => {
  const readiness = deriveEnterpriseTrustReadinessState({
    canViewSettings: true,
    canExportData: true,
    canViewAppHealth: true,
    canViewSupport: true,
  ...
  const packet = buildEnterpriseTrustReviewPacket(readiness, {
    companyName: "ABC Builders",
    userName: "Alex Owner",
    generatedAt: "2026-05-17T12:05:00.000Z",
  });

  assert.match(packet, /Apex HQ Pilot Trust Review Packet/);
  assert.match(packet, /Workspace: ABC Builders/);
  assert.match(packet, /Generated by: Alex Owner/);
  assert.match(packet, /Package: Premium/);
  assert.match(packet, /Role changed/);
  assert.match(packet, /copy-only owner\/admin review packet/i);
  assert.match(packet, /not a SOC 2 report/i);
  assert.doesNotMatch(packet, /certified|guaranteed|bank-level/i);
});
