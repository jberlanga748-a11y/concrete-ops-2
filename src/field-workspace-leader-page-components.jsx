...
function OfficeFieldWorkspaceLeaderPage({
  ...
  const fieldTools = [
    ...
    { id: "prePour", label: "Job Prep", helper: "Readiness checks", icon: "clipboard", tone: "amber", count: normalizeObjectArray(prePourChecklists).filter((checklist) => !checklist.archivedAt).length, disabled: !permissions.prePour.canView },
    { id: "postPour", label: "Closeout", helper: "Finish checks", icon: "clipboard", tone: "amber", count: normalizeObjectArray(postPourChecklists).filter((checklist) => !checklist.archivedAt).length, disabled: !permissions.postPour.canView },
