...
test("Apex Control Room route is extracted and wired through App", () => {
  ...
  assert.match(componentSource, /const APEX_COCKPIT_SILENCE_MS = 1800;/);
  assert.match(componentSource, /const APEX_COCKPIT_MIN_TURN_MS = 1200;/);
  ...
  assert.match(componentSource, /const shouldUseNativeVoice = cockpitNativeVoiceReady\s*&& \(!canUseCockpitRecorder \|\| cockpitMicPermissionState === "denied"\);/);
  ...
  assert.match(componentSource, /Apex finished speaking and is quiet until John starts the next turn/);
  ...
  assert.match(componentSource, /const \[cockpitAutoListening, setCockpitAutoListening\] = useState\(false\);/);
  assert.match(componentSource, /const \[cockpitConversationMode, setCockpitConversationMode\] = useState\(false\);/);
  assert.match(componentSource, /const cockpitAutoListeningRef = useRef\(false\);/);
  ...
  assert.match(componentSource, /Voice turn settled\. Apex is quiet until John starts the next turn/);
  assert.match(componentSource, /Voice benchmark armed\. Press Resume Voice/);
  assert.doesNotMatch(componentSource, /shouldReopenVisibleNative/);
  assert.doesNotMatch(componentSource, /openCockpitVoiceSession\(\{ automatic: true/);
  ...
  assert.match(componentSource, /I missed that\. Press Resume Voice and say it again\./);
