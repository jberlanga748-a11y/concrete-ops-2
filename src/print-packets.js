...
function fieldQuantityRecordsFromEstimate(printModel = {}) {
  return safeArray(printModel.lineItems).map((item) => {
    const quantity = cleanPacketText(item.quantity);
    const unit = cleanPacketText(item.unit);
    return {
      title: cleanPacketText(item.description) || "Estimate line item",
      meta: [
...
function fieldTakeoffReferenceRecords(backup = {}) {
  const takeoffRecords = safeArray(backup.takeoffRows).map((row) => {
    const quantity = cleanPacketText(row.quantity);
    const unit = cleanPacketText(row.unit);
    const source = cleanPacketText(row.source);
    return {
      title: cleanPacketText(row.item) || "Takeoff item",
      meta: [
        quantity ? `Qty ${quantity}` : "",
        unit,
  ...
  const referenceRecords = safeArray(backup.referenceRows).map((row) => {
    const source = cleanPacketText(row.source);
    return {
      title: cleanPacketText(row.fileName) || "Reference attachment",
      meta: [
        cleanPacketText(row.referenceType),
        source ? `Source ${source}` : "",
      ].filter(Boolean),
    };
  });
...
export function deriveEstimateForemanHandoffPacket({
  ...
  if (!estimate) {
    return buildPacket({
      ...
      logoImageUrl: companyProfile.logoImageUrl || "",
      companyProfileRows: deriveCompanyProfileRows(companyProfile),
      footerNote: printPacketFooter || "Generated by Apex HQ for field handoff review.",
      disclaimerNote: printPacketDisclaimer,
  ...
  ].filter((section) => cleanPacketText(section?.text));
  ...
  return buildPacket({
    ...
    logoImageUrl: companyProfile.logoImageUrl || "",
    companyProfileRows: deriveCompanyProfileRows(companyProfile),
    footerNote: printPacketFooter || "Generated by Apex HQ as a field-safe handoff. Review scope, quantities, exclusions, and change-order watchouts before work begins.",
    disclaimerNote: printPacketDisclaimer,
    title: "Foreman Handoff Packet",
    ...
    sections: [
      ...
      })),
      {
        ...
        type: "records",
        description: "Estimate line items shown for field planning only. Pricing, totals, margin, and customer terms are intentionally excluded.",
