...
test("deriveApexControlRoomState builds private operator status from visible state", () => {
  ...
  assert.equal(state.voiceInterface.providerStatus, "Push-to-talk review");
  ...
  assert.equal(state.voiceInterface.modes.some((item) => item.id === "push-to-talk" && item.status === "Click-to-record"), true);
  ...
  assert.equal(state.voiceInterface.safetyRows.some((item) => item.id === "no-microphone" && item.status === "Push-to-talk only"), true);
