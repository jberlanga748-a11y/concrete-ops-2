...
test("desktop Sidebar is extracted from App and keeps filtered navigation inputs", () => {
  ...
  assert.match(appSource, /<Sidebar[\s\S]*active=\{active\}[\s\S]*setActive=\{setActive\}[\s\S]*navGroups=\{visibleNavGroups\}[\s\S]*brandAssets=\{APEX_BRAND_ASSETS\}[\s\S]*appName=\{APP_NAME\}[\s\S]*workspaceLabel=\{isApexOsShell \? "Apex OS" : "Team workspace"\}[\s\S]*\/>/);
  ...
  assert.match(navSource, /firstVisible\(\["apexControlRoom"\]\)/);
