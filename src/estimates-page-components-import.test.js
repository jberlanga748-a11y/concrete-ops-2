...
test("Estimates page route shell is extracted and lazy-loaded out of App", () => {
  ...
  assert.match(estimatesPageSource, /Use Takeoff inside Estimates/);
  assert.match(estimatesPageSource, /Save as Draft Estimate/);
  assert.match(estimatesPageSource, /Back to mobile estimates/);
  assert.match(estimatesRouteSource, /PDF plan up to 50MB or an image plan up to 10MB/);
  assert.match(estimatesRouteSource, /Choose PDF \/ Image Plan/);
  ...
  assert.match(fs.readFileSync(new URL("./estimator-mobile-pipeline-components.jsx", import.meta.url), "utf8"), /Takeoff tool/);
