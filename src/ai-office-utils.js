...
export function deriveAiOfficeAgentCommandCenter({
  ...
  const workflowCards = [
    ...
    (permissions?.contactHistory?.canView || permissions?.customerPortal?.canPreview) ? {
      id: "communications-review",
      title: "Communications",
      helper: "Review contact history, manual drafts, customer portal approvals, and customer comments without sending messages or creating portal access.",
      icon: "quote",
      badge: "Manual review",
      tone: "blue",
      actionLabel: "Open communications",
      moduleId: "communications",
      recordType: "communicationsReview",
    } : null,
    permissions?.settings?.canView ? {
      id: "billing-prep",
      title: "Billing prep",
      helper: "Review package state, manual invoice prep, payment readiness, and closeout billing candidates without live billing or provider writes.",
      icon: "dollar",
      badge: readyToBill ? `${readyToBill} ready` : "Provider-ready",
      tone: readyToBill ? "green" : "orange",
      actionLabel: "Open billing prep",
      moduleId: "settings",
      recordType: "billingPrep",
    } : null,
