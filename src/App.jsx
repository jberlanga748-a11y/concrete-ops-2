...
import { JobsPage, JobsTablePolished, StartupStatusBadge } from "./jobs-page-components";
...
const LEGACY_SESSION_TOKEN_KEY = "apex-hq/session-token";
...
function TopBar({ active, setActive, stats, user, onLogout, syncing, saveSummary, navItems, permissions, companyName, companies = [], currentCompanyId = "", onSelectCompany, notificationSource = {}, onOpenPath, sessionToken = "", logoInitials = DEFAULT_LOGO_INITIALS, assistantState = null, onOpenAssistant = null }) {
            ...
            >
              <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 break-words text-xs font-bold text-slate-500">{row.id} Â· {jobNextStep(row)}</p>
                </div>
            ...
            return (
              <tr key={row.id} onClick={() => onSelect(row.id)} className={`cursor-pointer transition hover:bg-blue-50/60 ${selected ? "bg-blue-50/80" : ""}`}>
                <td className="px-4 py-3">
                  ...
                  <p className="text-xs font-bold text-slate-500">{row.id} Â· {jobNextStep(row)}</p>
                </td>
            ...
            <button type="button" onClick={() => onSelectCustomer(related.customer.id)} className="w-full rounded-2xl border border-blue-100 bg-blue-50/60 p-4 text-left hover:bg-blue-50">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold text-slate-500">{related.customer.city || "No city"} Â· {related.customer.id}</p>
                </div>
...
function DashboardOfficeQueueCard({
  ...
  const visibleItems = normalizeObjectArray(items).slice(0, 2);
...
function leadNeedsMobileFollowUp(lead = {}, today = todayDateInputValue()) {
  const status = normalizeTodayStatus(lead.status);
  const due = String(lead.followUpDueAt || "").slice(0, 10);
  return status === "new" || isLeadFollowUpDue(lead, today) || Boolean(due && due <= today);
}

function leadMissingMobileInfo(lead = {}) {
...
function buildEstimatorMobilePipelineQueue({ leads = [], estimates = [], customers = [], companyName = DEFAULT_COMPANY_NAME } = {}) {
  ...
  visibleEstimates
    .filter((estimate) => normalizeTodayStatus(estimate.status) === "draft")
    .slice(0, 3)
    .forEach((estimate, index) => addRow({
...
function EstimatorMobilePipelinePage({
  user,
...
function CommandCenterRoutePage(props) {
      ...
      <div className="rounded-2xl border border-blue-100 bg-white p-3">
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs font-bold text-slate-500">Manual copy only â€” no message is sent from Apex HQ.</p>
          </div>
    ...
    <div className="mt-4 rounded-3xl border border-amber-100 bg-amber-50/60 p-4">
      <div className="flex min-w-0 flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm leading-6 text-slate-700">Manual copy only â€” Apex HQ does not send email, SMS, or calls from this panel.</p>
        </div>
...
function MainContent(props) {
  ...
  if (active === "commandCenter") return <CommandCenterRoutePage {...props} />;
...
const OWNER_ADMIN_MOBILE_MORE_ORDER = [
  { id: "jobs", label: "Jobs", icon: "briefcase" },
  { id: "schedule", label: "Schedule", icon: "calendar" },
  { id: "reports", label: "Reports", icon: "document" },
...
const ESTIMATOR_MOBILE_NAV_ORDER = [
...
const ESTIMATOR_MOBILE_NAV_ROUTES = new Set(["leads", "estimates", "customers", "communications"]);
...
function getEstimatorMobileNavItems(visibleNavItems = []) {
  const visibleById = new Map((visibleNavItems || []).map((item) => [item.id, item]));
