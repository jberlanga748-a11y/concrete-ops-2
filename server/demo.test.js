...
test("demo mode seeds fake users and the full office-to-field workflow story", async () => {
  ...
  try {
    ...
    assert.equal(adminBootstrap.companySettings.companyName, "Concrete Ops Demo Company");
    assert.equal(adminBootstrap.companySettings.businessEmail, "office@concreteopsdemo.com");
    assert.equal(adminBootstrap.companySettings.printPacketFooter, "Generated by Concrete Ops for job documentation, field reports, and closeout records.");
    assert.ok(adminBootstrap.dailyReports.some((report) => report.id === "DR-DEMO-003" && report.status === "submitted"));
