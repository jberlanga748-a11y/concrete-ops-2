...
export function getAllowedModuleIds(user, companySettings = DEFAULT_COMPANY_SETTINGS) {
  ...
  if (isOwner(user) || isAdministrator(user) || isOperationsManager(user)) {
    ["dashboard", "commandCenter", "leads", "customers", "estimates", "jobDraftImports", "jobs", "schedule", "time", "reports", "uploads", "deliveryTickets", "changeOrders", "employees", "incidents", "toolbox", "ppe", "prePour", "postPour", "calculator", "settings", "copilot"].forEach((moduleId) => modules.add(moduleId));
