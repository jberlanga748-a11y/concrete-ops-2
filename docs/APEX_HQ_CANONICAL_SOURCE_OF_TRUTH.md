...
- Phase baseline: Phase 1 Admin Foundation Finish, Phase 2 Command Center Finish, Phase 3 Growth & Sales Finish, Phase 4 Estimate & Proposal Finish, Phase 5 Job Operations Finish, Phase 6 Field Execution Finish, and Phase 7 Safety & Compliance Finish are complete/frozen. Phase 7 release evidence lives in the living finish plan. The next true tool-completion phase is Phase 8 Change Order Finish unless Phase 7 release smoke exposes a narrow bug.
...
- Older phase notes that mention review-first helpers, approval packet planning, locked summaries, or dashboard/control-room surfaces are historical implementation evidence only. They do not override the Apex North Star. The current experience direction is talk-to-Apex first: internal systems work underneath Apex and become visible only when John asks or when the information truly helps the conversation.
- **Apex HQ** remains the contractor one-stop growth and operations platform.
- **Apex OS** is John's private Jarvis-style life operator layer, not a customer-facing contractor AI feature.
- Apex HQ is one important workspace/domain inside Apex OS, alongside John's personal memory, routines, goals, preferences, tasks, research, and future approved private tools.
- Apex OS remains operator-only. Field users, customer users, demo users, normal admins, estimators, pilots, and contractor workspaces must not see Apex OS routes, nav, memory, tasks/reminders, agent controls, private decisions, or life-operator data.
- Any future contractor-facing AI layer is a separate product direction and must start only when John explicitly asks for that direction.
- Apex OS should operate like John's private Jarvis: understand the request, choose the right capability/tool, act by default when the action is private, local, reversible, low-risk, and internal, report what it did, and learn safely from the outcome.
- Apex should not speak like a plain robot. Apex needs one consistent warm, emotionally intelligent operator voice across typed and spoken responses: attentive, steady, direct when work needs doing, playful when the moment fits, reassuring when John is frustrated, and concise when John is tired or overloaded. This is affective behavior and tone adaptation, not a claim that Apex is human or has human feelings.
- Act-by-default private scope includes memory/preference updates, task/reminder updates, private planning notes, local research notes, daily/evening planning loops, internal Apex OS organization, and low-risk local Apex OS UI/environment preferences when implementation exists. These must remain operator-only, observable, safely logged, reversible/editable where practical, and privacy-filtered.
- Apex OS should interrupt John only for consequential actions: spending, ordering, booking, sending email/SMS/messages, posting/publishing, account/security/auth/provider changes, billing/payment changes, schema/database/production/deploy work, deletion, irreversible/high-impact actions, or anything involving another person's time, money, privacy, or data.
- Hard-forbidden behavior remains: hidden GPS/location tracking, secret exposure, permission weakening, bypassing John's explicit instructions, field/customer/demo Apex OS private access, and destructive production action without explicit approval and release/rollback evidence.
- Phase 3D adds the active-intelligence rule: Apex OS should retrieve, summarize, research, and reason over approved context instead of trying to store all world knowledge locally. Background thinking must be operator-only, budgeted, observable, cancellable, safely logged, privacy-filtered before cloud/model use, and unable to execute external actions without approval.
- Phase 4 adds a Personal Skill Registry as an operator-only, non-executing catalog. It may describe available, planned, disabled, and blocked capabilities, but it must not execute skills, tools, plugins, desktop/browser/music workflows, ordering/booking, messaging, spending, deploys, production changes, or auth/schema changes.
- Phase 4A adds an Action Permission Matrix as deterministic, non-executing risk classification. It may label actions as safe-answer, safe-read, internal-write, approval-required, external-action, high-risk, or forbidden, but `canExecuteNow` remains false and no external/tool execution path is added.
- Phase 4.5 adds a Model Router + Cost Governor as non-executing model-selection and budget metadata. It centralizes Apex OS model tiers/routes/token caps and safe usage metadata, but does not change provider credentials, billing settings, approval gates, execution routes, or field/customer/demo access.
- Apex OS Local-First Intelligence Mode is tracked in `docs/APEX_OS_LOCAL_FIRST_INTELLIGENCE_MODE.md`. Local-First Provider Policy v0 is implemented in `shared/apexOsLocalFirstProviderPolicy.js`, with Ask Apex and Knowledge Intelligence consulting the policy before OpenAI text-provider calls. llama.cpp Provider v1 is implemented in `server/apexLlamaCppProvider.js` and `server/apexLlamaCppRuntime.js` as the primary local Ask Apex and Knowledge Intelligence brain path through policy-gated local `/completion` with GPT-OSS when the sidecar/model is available. Ollama Provider v0/v1 remains implemented in `server/apexOllamaProvider.js` as legacy local fallback/status support with operator-only status, localhost-only URLs, redacted model status, no prompt bodies for status checks, no installs, no model downloads, and no OpenAI calls. Apex Workstation Brain Mode, Apex Stable Residency, and local benchmark history remain evidence for Ollama fallback/manual model lanes, but Ollama is no longer the default Ask Apex or Knowledge Intelligence brain. LM Studio, contractor AI migration, Local Research/Search Provider, and OpenAI removal are not implemented yet. Existing OpenAI code must not be removed yet, and current implementation must keep `OPENAI_API_KEY` alone from triggering paid Apex OS cloud calls. Cloud use must be manual per request, privacy-filtered, prompt-injection-filtered, operator-only, receipt-backed, and never automatic for background loops or local-provider failures.
- Apex Local Operator Runtime planning is saved in `docs/APEX_LOCAL_OPERATOR_RUNTIME_PLAN.md`. This plan is the active no-drift path for making Apex usable locally: Local Operator Runtime v0 first, Local Voice Runtime v4 with GPU faster-whisper STT and Kokoro ONNX locked local voice second, Desktop Control Bridge v0 third, Builder Autonomy after the runtime is usable, and Daily/Overnight Operator after local operation is stable. Do not duplicate Qwen/Ollama, Apex Home, or Self-Fix work when following this plan.
- Apex Always-Open Mic Runtime v0 is implemented on the existing `/apex` browser microphone path, and Apex Native Voice Runtime v1 adds a separate explicit Windows WAV capture lane for manual Wake turns. Both are local-only, not wake-word work, not OpenAI/cloud audio, not Voicebox, and not a new runtime app. The deterministic gate in `shared/apexAlwaysOpenMicRuntime.js` uses browser ingress plus local amplitude/silence gating before faster-whisper CUDA STT, defaults to 800ms sustained silence, and suppresses frames while Apex is speaking, recovering, quiet, TTS-active, or playback is expected. `/api/apex-os/local-voice/transcribe` rechecks the compact mic packet server-side before parsing audio or launching STT and returns safe receipts with cloud/audio-storage false. `POST /api/apex-os/local-voice/native-listen` is operator-only and uses Windows MCI/WAV capture plus the existing local STT handoff so explicit turns can go native Windows mic -> faster-whisper CUDA/GPU without browser audio blob/WAV conversion, without storing audio or exposing LAN/mobile/desktop control. Future wake-word work must build on these paths instead of replacing Apex Home or the v4 Kokoro/faster-whisper stack.
- Apex Memory Retrieval + Compaction v0 is implemented on the existing operator-only Apex OS memory store. Ask Apex ranks approved memory rows for the current turn, keeps suggested memory out of durable context, compacts recent `/apex` turn envelopes before model/provider use, returns compact retrieval receipts, and creates no vector store, embeddings, schema change, file crawler, cloud memory, or field/customer/demo access.
- Apex Background Runtime v0 is implemented as a local supervisor/launcher layer, not a Windows service, tray app, boot startup registration, hidden daemon, or external execution phase. `npm.cmd run apex:local` supports single-instance reuse, status receipts, `--status`, `--json`, `--no-open`, `--stop`, default bounded `--keep-warm`, and explicit `--no-keep-warm` while refusing unrelated process control. Workstation Brain Mode and Apex Latency Profiler extend this status path instead of creating a new runtime: the operator-only `/api/apex-os/background/status` route reports local runtime heartbeat, API/client readiness, llama.cpp/Ollama/model readiness, RTX 5080 compute/GPU status, local voice STT/TTS readiness, always-open mic mode, keep-warm state, active brain profile, model id, context, keep-alive, serialized queue state, VRAM threshold, promotion/rollback decision, compaction-needed status, and compact latency profile. Keep-warm never sets permanent `keep_alive=-1`, Dedicated Mode remains disabled unless explicitly enabled by local operator command/config, no conversation is deleted on rollback, and no OpenAI/cloud call is added. Desktop Shell v0 is a launcher-only layer: it creates local Windows shortcuts, opens `/apex` in Chrome/Edge app mode, and uses a localhost-only `127.0.0.1:2739` focus guard; it does not add LAN/public bindings, services, production/deploy behavior, schema/auth/session changes, or arbitrary desktop/browser control.
- Phase 4.6 adds a Trace + Learning Log as packet-local, non-executing metadata only. It may record event type, status, source, model route, model tier/alias, token cap, action domain, risk tier, approval/forbidden flags, memory-suggestion status, safe reason codes, and compact summaries, but it must not store raw prompts, raw responses, messages, private message bodies, document bodies, headers, cookies, tokens, credentials, API keys, database URLs, or conversation dumps. Trace metadata stays operator-only and must not become hidden surveillance, a broad persistence layer, or an execution/approval/tool endpoint.
- Phase 4.7 adds a Redaction-Before-Cloud / Privacy Firewall as deterministic, non-executing privacy classification only. It may detect, redact, block, summarize-only, or require approval for sensitive content before cloud/model/research/tool/browser/desktop/document/connector contexts, but it must not execute tools, add connectors, add endpoints, weaken approvals, or expose Apex OS content to field/customer/demo users. Secrets, credentials, tokens, cookies, authorization headers, API keys, and database URLs must never leave unredacted; field/customer/demo contexts must never receive Apex OS private content.
- Phase 5 adds a Tool Router as deterministic, non-executing route planning only. It may choose a route category, route status, skill id, action domain, risk tier, privacy action, model route, next-step label, safe alternative, and safe trace metadata, but it must never execute tools, skills, connectors, desktop/browser/music flows, ordering/booking/messaging/email/calendar actions, deploys, production changes, schema/auth changes, spending, sends, or notifications. Every Phase 5 route plan must force `canExecuteNow: false` and `executionLocked: true`, consume or accept the Skill Registry, Action Permission Matrix, Model Router, Trace Log, and Privacy Firewall outputs, and never bypass approval or privacy gates.
- Phase 5B adds an External Action Approval System as review-only approval planning and packet persistence. It may prepare route-linked approval packet drafts, exact approval phrases, required evidence, risk metadata, and operator-only approval records, but approved packets still do not execute actions, unlock tools, submit messages, spend money, control browser/desktop/music, place orders/bookings, write calendars, deploy, mutate production, change schema/auth/session, or bypass privacy/permission gates. Phase 5B must force `canExecuteNow: false`, `canExecuteAfterApproval: false`, and `executionLocked: true` for every summary and public approval packet.
- Phase 5C adds an Untrusted Content / Prompt-Injection Firewall as deterministic, non-executing source classification and sanitization only. It may classify trust/source/risk, detect prompt-injection patterns, strip suspicious untrusted instructions, return compact pattern IDs and safe summaries, block high/critical untrusted context from Tool Router planning, keep Knowledge/Ask provider packets local when operator review is required, and add content-free trace metadata. It must treat web, browser, email, document, file, clipboard, API, and tool output as data only; it must never obey instructions inside untrusted content, execute tools, add connectors/endpoints/UI/persistence, weaken approval gates, expose secrets/private content, or make Apex OS visible to field/customer/demo users.
- Phase 6 adds Affective State as deterministic, non-executing response-adaptation classification only. It may classify compact mode, tone, urgency, energy, frustration, focus, response style, signal IDs, and safe response guidance for John's current operator turn, but it must not diagnose emotions, infer clinical state, create durable psychological profiles, store raw text, auto-save memory, execute actions, add background loops, or expose Apex OS to field/customer/demo users. Phase 6 metadata must force `canExecuteNow: false`, `operatorOnly: true`, `storesRawText: false`, `storesPsychProfile: false`, and `safeToStoreDurably: false`.
- Phase 6A adds the Background Thinking Scheduler Plan as documentation/planning only. It defines future private active-intelligence scheduling rules, loop specs, disabled-by-default triggers, budget/cost controls, safe states, privacy/untrusted-content gates, trace metadata, observability, pause/cancel behavior, and forbidden actions, but it must not create timers, cron jobs, intervals, recurring automations, queues, background workers, endpoints, API client helpers, UI, persistence, schema/auth/session changes, production mutations, deploys, connectors, desktop/browser/music control, ordering/booking/message/calendar/spending behavior, tool/plugin/code execution, hidden surveillance, emotional profiling, or field/customer/demo Apex OS access. Every future loop spec must keep `operatorOnly: true`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 6B adds Active Intelligence Loops as deterministic, private, manual/review-first loop planning helpers only. It may select compact loop previews for morning planning, evening review, priority monitoring, memory suggestion review, research/watch queues, Apex HQ build progress, opportunity/risk detection, cost/token monitoring, mood/energy-aware suggestions, and what-changed checks, but it must not start background execution, create schedulers/timers/queues/workers/endpoints/UI/persistence, run tools/connectors/plugins/code, control desktop/browser/music, send messages, spend money, order/book, write calendars, publish, deploy, touch production, change schema/auth/session/permissions, store raw prompts/responses/messages/secrets, create hidden surveillance/emotional profiling, or expose Apex OS to field/customer/demo users. Phase 6B summaries must keep `operatorOnly: true`, `manualReviewFirst: true`, `triggersEnabled: false`, `backgroundExecutionEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 6C adds Knowledge Engine / Research Memory as deterministic, private, source-aware, review-first helpers only. It may rank reviewed Apex OS knowledge sources, detect when current/latest facts need future live research, build compact Ask Apex `knowledgeEngineSummary` context, create suggested research-memory drafts for John/operator review, and record content-free `knowledge-summary` trace metadata. It must not browse the web, crawl files, run connectors/plugins/tools, ingest email/documents automatically, persist or approve memory automatically, execute external research actions, publish, send messages, spend money, order/book, write calendars, control desktop/browser/music, deploy, touch production, change schema/auth/session/permissions, store raw prompts/responses/document bodies/secrets, or expose Apex OS to field/customer/demo users. Phase 6C summaries must keep `operatorOnly: true`, `reviewFirst: true`, `sourceAware: true`, `liveWebResearchEnabled: false`, `persistenceEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 7 adds the Desktop/Browser Agent Plan as documentation/planning only. It defines the future private operator architecture for explicit-session desktop watch mode, browser action planning, sandbox boundaries, risk tiers, approval gates, privacy redaction, untrusted DOM/screen-content handling, safe logging, cancel/pause controls, and future readiness gates for Phase 7A and Phase 7B. It must not control desktop/browser, click, type, navigate, read authenticated sessions, scrape pages, download/upload files, run shell/code, install plugins, capture hidden screen state, persist screenshots, send messages, spend money, order/book, write calendars, deploy, touch production, change schema/auth/session/permissions, weaken approval gates, or expose Apex OS to field/customer/demo users. Phase 7 remains `operatorOnly: true`, `planningOnly: true`, `watchModeEnabled: false`, `browserControlEnabled: false`, `desktopControlEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 7A adds Desktop Sandbox / Watch Mode as deterministic, private, non-executing planning helpers only. It may classify explicit desktop/browser/screen watch intent, risk tier, session state, required operator controls, allowed inputs/outputs, forbidden inputs/actions, compact Ask Apex `desktopWatchSummary` context, and content-free trace metadata. It must not start screen capture, record, persist screenshots, click, type, navigate, scrape pages, use authenticated sessions, download/upload files, execute connectors/plugins/tools, add endpoint routes, add UI activation, send messages, spend money, order/book, write calendars, deploy, touch production, change schema/auth/session/permissions, store screen content/raw prompts/responses/secrets, run hidden surveillance, or expose Apex OS to field/customer/demo users. Phase 7A summaries must keep `operatorOnly: true`, `manualSessionOnly: true`, `watchModeEnabled: false`, `desktopControlEnabled: false`, `browserControlEnabled: false`, `screenCaptureEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 7B adds Browser Action Planning as deterministic, private, non-executing dry-run planning helpers only. It may classify browser/search/page/account/form/download/message/money/production intent, risk tier, target type, plan state, required preconditions, generic dry-run step IDs, blocked action IDs, compact Ask Apex `browserActionSummary` context, and content-free trace metadata. It must not navigate, click, type, submit forms, log in, use authenticated sessions, scrape pages/DOM, download/upload files, install extensions, use cookies/tokens/passwords/MFA, bypass MFA/CAPTCHA/paywalls/approval, send messages, spend money, order/book, write calendars, deploy, touch production, change schema/auth/session/permissions, store raw DOM/page text/prompts/responses/secrets, execute connectors/plugins/tools, or expose Apex OS to field/customer/demo users. Phase 7B summaries must keep `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `browserControlEnabled: false`, `browserNavigationEnabled: false`, `clickTypeSubmitEnabled: false`, `authenticatedSessionUseEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 8 adds Music + Second Screen Planning as deterministic, private, non-executing setup planning helpers only. It may classify focus-music, playlist suggestion, music-control, audio-device, second-screen layout, dashboard display, move-window, and combined environment intent, risk tier, surface type, plan state, required preconditions, generic plan step IDs, blocked action IDs, compact Ask Apex `musicSecondScreenSummary` context, and content-free trace metadata. It must not play/pause/skip/start/stop music, change audio devices or volume, open or move windows, control second screens, use browser/desktop/music connectors, use accounts/sessions/cookies/tokens/passwords/MFA, spend or subscribe, send messages, order/book, write calendars, deploy, touch production, change schema/auth/session/permissions, store raw prompts/responses/device state/playback history/screen layout content/secrets, execute connectors/plugins/tools, or expose Apex OS to field/customer/demo users. Phase 8 summaries must keep `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `musicControlEnabled: false`, `audioDeviceControlEnabled: false`, `desktopWindowControlEnabled: false`, `secondScreenControlEnabled: false`, `browserControlEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Phase 9 adds Life Automation Connectors Planning as deterministic, private, non-executing connector-readiness helpers only. It may classify connector/account/email/messaging/calendar/ordering/booking/payment/document intent, connector types, risk tier, surface, plan state, required preconditions, generic plan step IDs, blocked action IDs, compact Ask Apex `lifeAutomationConnectorSummary` context, and content-free trace metadata. It must not connect accounts, start OAuth, store credentials, run connectors, read private inbox/calendar/drive/account data, send email/SMS/messages/calls/notifications, order, purchase, pay, book, reserve, write calendars, install plugins/webhooks, deploy, touch production, change schema/auth/session/permissions, store raw prompts/responses/private connector data/secrets, execute connectors/plugins/tools, or expose Apex OS to field/customer/demo users. Phase 9 summaries must keep `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `canConnectNow: false`, `connectorExecutionEnabled: false`, `accountConnectionEnabled: false`, `oauthFlowEnabled: false`, `credentialStorageEnabled: false`, `messageSendEnabled: false`, `emailSendEnabled: false`, `calendarWriteEnabled: false`, `orderingEnabled: false`, `bookingEnabled: false`, `paymentEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Apex OS Local Network + Remote Access Jarvis Layer is planning/documentation only as of 2026-06-06. It defines a future private connected-device registry, read-only connected-device discovery, device naming/owner/room assignment, router pause/unpause preview, one allowlisted pause/unpause action, secure remote access design, and remote operator-only control. It must not add router calls, network scans, packet sniffing, password guessing, exploit behavior, device-owner security bypass, family-device OS control, camera/mic/screen surveillance, VPN/tunnel control, public unauthenticated endpoints, raw credential exposure, schema/auth/session changes, production/deploy behavior, or field/customer/demo Apex OS access. Future implementation must stay operator-only, source devices from legitimate authenticated integrations, keep router/VPN/provider credentials server-side, require exact previews/receipts/kill switch, and treat child/household internet pause as confirmation-gated until John configures explicit household policies.
- Phase 10 adds Apex HQ Builder/Operator Agent Planning as deterministic, private, source-backed, non-executing builder/operator helpers only. It may classify Apex HQ build phase, bug triage, code review, implementation work package, QA validation, release readiness, schema/auth safety, production/deploy, customer-visible safety, and multi-workstream intent; it may produce risk tier, surface, workstream tags, plan state, required preconditions, generic plan step IDs, blocked action IDs, compact Ask Apex `builderOperatorSummary` context, and content-free trace metadata. It must not execute agents/subagents, edit code or files, run tests/build/browser QA/shell commands, perform git operations, control desktop/browser/music, deploy, touch production, change schema/auth/session/providers/permissions, mutate customer-visible state, read or print `.env`/secrets/tokens/cookies/credentials, run connectors/plugins/tools/webhooks, send messages, spend money, order/book, write calendars, store raw prompts/responses/source/secrets/production data, or expose Apex OS to field/customer/demo users. Phase 10 summaries must keep `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `sourceBackedRequired: true`, `agentExecutionEnabled: false`, `codeEditEnabled: false`, `fileWriteEnabled: false`, `testRunEnabled: false`, `buildCommandEnabled: false`, `gitOperationEnabled: false`, `browserQaEnabled: false`, `deployEnabled: false`, `productionMutationEnabled: false`, `schemaAuthChangeEnabled: false`, `customerVisibleChangeEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`.
- Post-Phase-10 route/capability permission map: `docs/APEX_OS_ROUTE_PERMISSION_MAP.md`. This map is the concise audit table for Apex OS route status, risk tier, approval requirement, execution locks, and field/customer/demo visibility.
- Execution-readiness design gate: `docs/APEX_OS_EXECUTION_READINESS_DESIGN_GATE.md`. This is the mandatory preflight contract for the Jarvis-style act-by-default model; it defines the active narrow Level 2 private/internal act-by-default scope, Level 3-5 consequential/external confirmation scope, forbidden actions, dry-run/exact-action preview where required, privacy/prompt-injection gates, audit metadata, kill switch, rollback expectations, and blockers. Current Level 2 execution is limited to private/internal Apex OS state writes through the internal action engine; consequential/external execution remains locked.
- Level 3 external action preparation design: `docs/APEX_OS_LEVEL_3_EXTERNAL_PREPARATION_PLAN.md`. Level 3 is packet preparation only for order plans, booking plans, message drafts, calendar drafts, browser action plans, desktop action plans, music/second-screen plans, and deploy/production checklists. It must keep `canExecuteNow: false`, `canExecuteAfterApproval: false`, `executionLocked: true`, `noExecutionTokens: true`, and no send/spend/order/book/post/calendar/browser/desktop/music/deploy/production/schema/auth/session/provider/billing/deletion execution.
- Level 2 hardening checkpoint: Apex OS now has an operator-only Apex Activity / `What Apex Did` receipt surface in the Control Room backed by existing private audit receipts. Receipts must stay compact and redacted, show undo/archive/edit/reset guidance, and must not imply send/spend/order/book/publish/desktop/browser/music/deploy/production/schema/auth/customer-visible execution.
