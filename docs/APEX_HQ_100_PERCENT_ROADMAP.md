...
Build 7B local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds authenticated server-owned customer portal access-record preparation endpoints that are Elite package gated, owner/admin role gated, company scoped, audit backed, non-redeemable, and locked as internal readiness evidence. It rejects raw tokens, public URLs, share links, customer logins, sends, invoice/payment fields, and still does not create a live customer portal, customer route, redeemable token, customer action, production deploy, secrets/config changes, or production data work.

Build 7C local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds locked access-record lifecycle behavior: revoke appends an internal audit event, expired status is derived at read time, duplicate revokes are blocked, and wrong-company and field-role direct API calls are denied. It still does not create a live customer portal, customer route, redeemable token, customer action, production deploy, secrets/config changes, or production data work.

Build 7D local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds the locked public customer portal route contract: `/portal/:accessId` returns locked metadata only, token redemption remains disabled, and missing, malformed, wrong-company, expired, and revoked denial cases are covered without serving customer data. It still does not create a live customer portal, redeemable token, customer session, customer action, production deploy, secrets/config changes, or production data work.

Build 7E local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds an authenticated internal owner/admin access-record packet endpoint that derives a customer-facing review packet from an active locked access record while blocking Basic/Premium, field roles, wrong-company users, expired records, and revoked records. It redacts token hash references, audit internals, internal notes, raw-token fields, and public URLs, and still does not expose the packet to customers, create a live portal, redeem tokens, accept customer actions, send messages, collect payment, deploy, change secrets/config, or touch production data.

Build 7F local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds an authenticated internal owner/admin share approval queue for active locked access records while blocking Basic/Premium, field roles, wrong-company users, unsafe external payload fields, expired records, and revoked records. Queue items are audit backed, packet-ready, and explicitly keep customer logins, public links, redeemable tokens, customer sessions, customer actions, customer messages, invoices, payments, deploys, secrets/config, and production data locked.

Build 7G local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds authenticated internal owner/admin review decisions for locked share approval requests while blocking duplicate reviews, unsafe external payload fields, field roles, and wrong-company users. Decisions can mark the packet ready for a future separately approved external gate, request changes, or reject the share review, but still keep customer logins, public links, redeemable tokens, customer sessions, customer actions, customer messages, invoices, payments, deploys, secrets/config, and production data locked.

Build 7H local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds an authenticated read-only owner/admin external-gate preflight for reviewed share approval requests while blocking unsafe external payload fields, field roles, and wrong-company users. The preflight reports internal prerequisites and exact separate approval phrase evidence, but still keeps external portal implementation, public links, redeemable tokens, customer sessions, customer actions, customer messages, invoices, payments, deploys, secrets/config, and production data locked.

Build 7I local completion evidence is tracked in `docs/APEX_HQ_CUSTOMER_PORTAL_TOKENIZED_READINESS_STATUS.md` and verified by `npm.cmd run verify:customer-portal-readiness`. It adds an authenticated owner/admin locked external execution contract for reviewed share approval requests, including Agent OS customer portal action mapping, per-company opt-in evidence, idempotency behavior, audit event, rollback behavior, and a hard-deny execution route. It still keeps external portal implementation, public links, redeemable tokens, customer sessions, customer actions, customer messages, invoices, payments, deploys, secrets/config, and production data locked.

Build 8A local completion evidence is tracked in `docs/APEX_HQ_COMMUNICATION_PROVIDER_READINESS_STATUS.md` and verified by `shared/communicationProviderReadiness.test.js` plus `server/agent-os.test.js`. It adds locked communication provider readiness and an outbound approval queue for email/SMS provider workflows, including consent source, opt-out, do-not-contact, template review, delivery history, idempotency, audit, rollback, provider readiness, and hard-deny execution behavior. It does not send email/SMS/portal notifications, submit bids, create invoices, collect payment, store provider secrets, deploy, change secrets/config, or touch production data.

Build 8B local completion evidence is tracked in `docs/APEX_HQ_COMMUNICATION_PROVIDER_READINESS_STATUS.md` and verified by `shared/communicationProviderReadiness.test.js` plus `server/agent-os.test.js`. It adds locked communication suppression records and locked delivery-attempt contracts for broader email/SMS provider workflows, including suppression reasons, normalized recipient enforcement, idempotency, audit, rollback, provider-readiness references, failure classes, and hard-locked provider request flags. It does not prepare provider requests, call unsubscribe endpoints, send email/SMS/portal notifications, submit bids, create invoices, collect payment, store provider secrets or raw provider responses, deploy, change secrets/config, or touch production data.

Build 8C local completion evidence is tracked in `docs/APEX_HQ_COMMUNICATION_PROVIDER_READINESS_STATUS.md` and verified by `src/communication-provider-readiness-utils.test.js`, `src/communications-page-shell.test.js`, and `npm.cmd run verify:leads`. It makes the locked communication provider readiness packet visible in the Communications screen, summarizes channel readiness, missing evidence, locked approvals, suppressions, and delivery-attempt contracts, and lets office users record selected-record suppression evidence without sending messages or calling provider unsubscribe endpoints. It does not prepare provider requests, send email/SMS/portal notifications, call provider unsubscribe endpoints, store provider secrets or raw provider responses, deploy, change secrets/config, or touch production data.

Build 8D local completion evidence is tracked in `docs/APEX_HQ_COMMUNICATION_PROVIDER_READINESS_STATUS.md` and verified by `src/communication-provider-readiness-utils.test.js`, `src/communications-page-shell.test.js`, `shared/communicationProviderReadiness.test.js`, `server/agent-os.test.js`, and `npm.cmd run verify:leads`. It exposes locked outbound approval queueing and locked delivery-attempt contract preparation in the Communications screen while preserving consent/template/human review blockers, idempotency, audit evidence, suppression checks, and provider request flags. It does not prepare provider requests, send email/SMS/portal notifications, call provider unsubscribe endpoints, store provider secrets or raw provider responses, deploy, change secrets/config, or touch production data.

Build 9A local completion evidence is tracked in `docs/APEX_HQ_SCHEDULING_GATE_READINESS_STATUS.md` and verified by `shared/agentOperatingSystem.test.js` plus `server/agent-os.test.js`. It adds a locked scheduling external-gate readiness packet and server-side preflight for visible company-scoped jobs, conflict checks, notification policy review, idempotency, adapter-readiness checks, and restore-from-audit planning. It does not mutate schedules, assign crew, change field visibility, notify crew/customers, write calendars, deploy, change secrets/config, or touch production data.

Build 9B local completion evidence is tracked in `docs/APEX_HQ_EXTERNAL_GATE_READINESS_STATUS.md` and verified by `shared/agentOperatingSystem.test.js`, `server/agent-os.test.js`, and `src/agent-os-ui-utils.test.js`. It exposes the Agent OS locked external-gate readiness deck in the console and adds review-only preflight endpoints for email, SMS, payment collection, customer portal actions, bid submission, and integration writes. It records target review, evidence checks, idempotency, adapter readiness, blockers, rollback notes, and blocked actions, but it does not prepare provider requests, send messages, collect payment, write customer portal data, mutate schedules, submit bids, write integrations, deploy, change secrets/config, or touch production data.

Build 9C local completion evidence is tracked in `docs/APEX_HQ_EXTERNAL_GATE_READINESS_STATUS.md` and verified by `shared/agentOperatingSystem.test.js`, `server/agent-os.test.js`, and `src/agent-os-ui-utils.test.js`. It exposes the Agent OS locked external-gate execution contract deck, idempotent contract routes, and hard-deny execute routes for email, SMS, payment collection, customer portal actions, bid submission, and integration writes. Contract routes can record redacted internal approval evidence only; execute routes remain locked and do not prepare provider requests, send messages, collect payment, write customer portal data, mutate schedules, submit bids, write integrations, deploy, change secrets/config, or touch production data.

Build 9D local completion evidence is tracked in `docs/APEX_HQ_EXTERNAL_GATE_READINESS_STATUS.md` and verified by `shared/agentOperatingSystem.test.js`, `server/agent-os.test.js`, and `src/agent-os-ui-utils.test.js`. It adds sandbox/internal adapter runs for customer portal action, SMS send, payment collection, integration write, and bid submission. Each run requires a prepared locked execution contract and records audit evidence for internal approval, test-recipient, sandbox payment, sandbox field-map, or packet-manifest review only. It does not prepare provider requests, send messages, collect payment, write customer portal data, mutate schedules, submit bids, write integrations, deploy, change secrets/config, or touch production data.
