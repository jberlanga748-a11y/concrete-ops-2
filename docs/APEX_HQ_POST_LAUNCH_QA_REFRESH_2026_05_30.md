...
2026-05-30 recovery: John approved the narrow production smoke-user/secret setup step. The `APEX_PRODUCTION_SMOKE_PASSWORD` GitHub Actions secret was rotated, the dedicated production smoke users were rotated, and `smoke.foreman@apexhq.app` was created as an active `Foreman` smoke user in the existing production smoke company context.
