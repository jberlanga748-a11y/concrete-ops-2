...
2026-06-09: Apex Trusted Desktop Entry v1 is the current private local desktop auth behavior. The dedicated Electron desktop app now requests `POST /api/apex-os/local-desktop-session` from the local API before loading `/apex`, installs the normal Apex auth cookies into the Electron session, and opens directly into the existing local operator account instead of showing the login screen. This endpoint is loopback-only, disabled in production, requires the dedicated desktop header, reuses an existing active `operatorAccess` Apex OS user, returns no bearer token, and preserves normal browser login behavior. This is a scoped local desktop auth/session bridge only: no schema change, no role promotion, no new account creation, no deploy, no production data, no secrets, no cloud/OpenAI/Groq, no LAN/public exposure, and no hidden microphone capture.

2026-06-09: Apex Dedicated Desktop App v1 is the current private local launch target. This PC is Apex's dedicated home, and `Apex.lnk` now launches `npm.cmd run apex:desktop`, which starts/reuses the local Apex runtime and opens `/apex` in a dedicated Electron desktop window instead of a Chrome/Edge app-mode bridge. Localhost remains internal plumbing: normal receipts report `homeBase.mode = apex-home-base-v1`, `homeBase.launch.userShouldSeeLocalhost = false`, `homeBase.launch.primaryCommand = npm.cmd run apex:desktop`, and `desktopApp.trueDesktopApp = true` in the desktop-app receipt. `npm.cmd run apex:local` remains the runtime/status/shortcut-refresh command underneath, while `--browser-tab` and the Chrome/Edge bridge remain troubleshooting fallbacks. The launcher still stays local-only, prepares llama.cpp GPT-OSS by default, keeps Ollama as legacy/manual fallback only, creates/refreshes local Windows shortcuts on Windows, and does not add a Windows service, boot registration, tray app, LAN/public binding, cloud fallback, deploy, production touch, or unrelated desktop control. The later Trusted Desktop Entry v1 above adds only the scoped loopback desktop session bridge.
...
2026-06-09: Apex Avatar-Only Context Window v1.1 is the current private local cockpit behavior. The default `/apex` surface is Apex's avatar plus one compact bottom-right context window for transcript, prompt, and answer. The home route does not render the operator sidebar or top identity/status header, and conversation-first CSS removes the cockpit border, frame chrome, avatar status label, old operator controls, status chips, diagnostics, voice details, skills, command stream, and stage panels. Those systems remain hidden machinery unless Apex is asked for that information. "Show diagnostics/status", "show voice", and "show skills/agents" now update the context window instead of turning the home surface back into a dashboard wall. "Clear screen" resets Apex back to the calm avatar-only surface. No model dropdowns, cloud calls, auth/session/schema changes, deployment, family-care integration, or permission changes were added.
...
- Phase 1 launch/home-base follow-up: the dedicated desktop window is now the launch target and it opens with a trusted loopback-only desktop session. The next refinement is to package it as a more durable PC-resident Apex app with quieter startup, no console flash, optional boot/login registration only after approval, and deeper native voice readiness. Runtime receipts must keep reporting the dedicated-app state and still keep localhost as internal plumbing.
