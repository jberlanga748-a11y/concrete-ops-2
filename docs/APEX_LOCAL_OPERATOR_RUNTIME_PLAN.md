...
2026-06-09: Apex Home Base v1 is the current private local operating rule: this PC is Apex's dedicated home. `npm.cmd run apex:local` now opens `/apex` through the temporary Chrome/Edge app-mode bridge by default so John does not have to work from a visible localhost tab. Normal runtime/background receipts now report `homeBase.mode = apex-home-base-v1`, `homeBase.identity.operatingRule = This PC is Apex's dedicated home.`, `homeBase.launch.userShouldSeeLocalhost = false`, and `desktopApp.target = apex-local-desktop-app` with `desktopApp.trueDesktopApp = false`. That lets Apex know the PC is home base now while staying honest that the current window is still a bridge, not the final dedicated desktop app. `--browser-tab` remains the explicit troubleshooting fallback when normal browser mic/session debugging is needed. The launcher still stays local-only, prepares llama.cpp GPT-OSS by default, keeps Ollama as legacy/manual fallback only, creates/refreshes local Windows shortcuts on Windows, and does not add a Windows service, boot registration, tray app, LAN/public binding, cloud fallback, deploy, schema/auth/session change, production touch, or unrelated desktop control.
...
- Phase 1 launch/home-base follow-up: Chrome/Edge app-mode is only the temporary bridge. The next refinement is a dedicated private Windows desktop app/window that does not require John to think about browser tabs or localhost, keeps the server bound locally, and still avoids services/boot hooks until approved. Runtime receipts must keep reporting the bridge/dedicated-app distinction until `desktopApp.trueDesktopApp` is actually proven true.
