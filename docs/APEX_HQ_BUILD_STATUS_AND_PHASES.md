...
- Production auth smoke design: prepared as an approval-ready plan only. `docs/apex-hq-production-auth-smoke-design.md` defines dedicated smoke-user, secret, command, pass/fail, workflow, and rollback requirements without creating users, setting secrets, enabling workflows, changing auth, or touching production.
- Production cold-start decision note: prepared as an approval-ready plan only. `docs/apex-hq-production-cold-start-decision.md` records the current `min_machines_running = 0` posture, read-only Fly status evidence, cold/warm `/api/ready` timing, and the gate for moving production or a pilot app to one always-on machine.
...
- Customer pilot readiness preflight: built so `npm run verify:pilot-readiness` runs the local non-deploying pilot gate: docs drift, pilot config safety, role gates, backup/export, restore drill, and frontend build.
...
- Docs drift guardrail: `npm run verify:docs` now scans root/demo docs and `docs/` for the old shared demo password and stale hard-coded readiness/release phrases; CI runs it with public intake and demo safety checks.
- Latest Fly demo smoke: `https://concrete-ops-demo.fly.dev/api/ready` returned ready/database ok, and GitHub Actions demo hosted smoke run `26140455523` passed on `main` at commit `74aa236` with non-auth route smoke plus auth/bootstrap smoke. Admin login/bootstrap were 145ms/274ms; employee login/bootstrap were 139ms/381ms; employee restricted-route checks ran.
...
- Latest local visual route sweep: `npm.cmd run build`, `npm.cmd run audit:visual-polish:chromium`, and `npm.cmd run audit:visual-polish:tablet` passed with 0 failures after starting the local app on port 4000. Evidence manifests: `ui-audit/visual-polish/2026-05-19T21-08-17-168Z/manifest.json`, `ui-audit/visual-polish/2026-05-19T21-08-42-617Z/manifest.json`, `ui-audit/visual-polish/2026-05-19T21-09-08-192Z/manifest.json`, and `ui-audit/visual-polish/2026-05-19T21-09-44-118Z/manifest.json`. Do not run desktop/phone and tablet sweeps in parallel against the same local demo session; a parallel run caused transient admin-tablet 401 console noise that cleared when tablet was rerun alone.
...
- Current demo smoke, monitoring, restore, pilot-doc, and CI hardening commits through `33e1eb1` added Opportunity Scout hosted smoke automation, demo package/cleanup safety scripts, Fly demo orchestration, latency budgets, pilot smoke runbooks, production readiness monitoring, scheduled demo hosted smoke, Docker readiness health checks, incident notes, restore-drill cadence, CI-covered local restore drill verification, demo smoke secret setup docs, monitoring upgrade planning, launch readiness reconciliation, customer pilot setup gates, production auth smoke design, production cold-start decision notes, customer pilot config verification, canonical manual smoke runbook cleanup, CI step-level timeout hardening, and demo hosted smoke step-level timeout hardening. Production remains locked unless a separate backup-first production release is approved.
