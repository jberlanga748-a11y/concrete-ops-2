...
- GitHub Actions demo smoke secret runbook: prepared, pushed, and CI-verified. `docs/apex-hq-github-actions-smoke-secrets.md` now defines how to configure, verify, rotate, and troubleshoot `APEX_SMOKE_PASSWORD` without storing secrets in repo/docs/chat. Current GitHub secret listing showed no repo secrets, so scheduled demo auth smoke will keep skipping until the secret is configured.
...
- Latest Fly demo smoke: `https://concrete-ops-demo.fly.dev/api/ready` returned ready/database ok, and `npm.cmd run smoke:hosted -- --base-url=https://concrete-ops-demo.fly.dev --skip-auth --json` passed health plus direct route checks for the main demo chain and supporting app routes. Auth smoke still waits on `APEX_SMOKE_PASSWORD`.
...
- Visual route sweep script cleanup: `npm run audit:visual-polish:full` now runs the stable Chromium desktop/phone sweep and tablet sweep sequentially to avoid local demo-session collisions during full visual QA.
