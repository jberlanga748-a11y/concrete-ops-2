...
- Apex OS Phase 17 Full Apex OS QA And Security Hardening: hard-finished locally on 2026-06-03 and pending production release, with final proof rows for role/field restrictions, customer/company isolation, direct-route blocking, source-backed answers, upload privacy, approval gates, build/test/release safety, desktop/mobile QA, production-preview smoke proof path, docs/memory drift, no secrets, risky-action bypass blocking, and the Apex OS access kill switch; focused tests, broad security/company-scope regression coverage after a transient company-scope startup rerun, role verification, production build, diff check, and isolated desktop/mobile/admin-blocked browser QA passed. No schema/auth/session change, provider setup, production mutation, customer-visible action, external send, ad spend, billing/payment, deploy/rollback execution from the UI, deletion, or irreversible action path was added.
