...
- Customer pilot config verifier: built so customer-specific Fly pilot configs can be checked locally before app/volume creation or deploy. `npm run pilot:verify-config -- --config=<customer-fly.toml>` rejects production/demo app names, production/demo volumes, demo seeding, demo package settings, missing `/api/ready` checks, and unsafe data-dir settings without touching Fly or production.
