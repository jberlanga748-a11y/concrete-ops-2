...
- Production auth smoke workflow: prepared as a manual, fail-closed GitHub Actions workflow. `.github/workflows/production-auth-smoke.yml` requires the exact `PRODUCTION_AUTH_SMOKE_APPROVED` confirmation phrase, an approved production URL, and the production-only `APEX_PRODUCTION_SMOKE_PASSWORD` secret before running login/bootstrap smoke with `--allow-production-auth`. It does not schedule itself, deploy, mutate app data, toggle packages, run Opportunity Scout, create users, set secrets, change auth, or touch production config. As of May 20, 2026, `APEX_PRODUCTION_SMOKE_PASSWORD` is not configured.
...
- Latest customer pilot readiness preflight: `npm.cmd run verify:pilot-readiness` passed locally on May 20, 2026 after the auth security test cleanup. Backup verification produced `app-data-20260520-055112Z.sqlite` / `.json`; restore drill produced `app-data-20260520-055113Z.sqlite` / `.json`, booted the restored SQLite copy, and confirmed `/api/ready` ready/database ok.
