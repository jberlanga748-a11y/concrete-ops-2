...
- Live demo smoke refresh: `https://concrete-ops-demo.fly.dev/api/ready` returned ready/database ok, and `npm.cmd run smoke:hosted -- --base-url=https://concrete-ops-demo.fly.dev --skip-auth --json` passed health plus direct route checks on May 19, 2026. Auth smoke remains unverified until `APEX_SMOKE_PASSWORD` is configured.
