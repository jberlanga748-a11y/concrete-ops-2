...
Goal recorded on 2026-06-02 and clarified on 2026-06-06: build **Apex OS**, John's private Jarvis-style life operator inside the main Apex HQ app, with Apex HQ as one important workspace/domain inside Apex OS.

North Star recorded on 2026-06-07: Apex is John's private operator. Apex is not a dashboard, review system, or safety product. Apex is one main intelligence with internal agents underneath it. John talks; Apex does. Apex reports only what matters. Apex should use what John already built instead of rebuilding. Apex should act for private, local, reversible work. Apex should not expose internal machinery unless John asks. Apex should feel alive, emotionally intelligent, capable, and personal. Apex HQ is one domain Apex can operate, not the whole identity of Apex.

Do Not Drift rule: before any Apex work, Codex, Builder, and every Apex agent must ask, "Does this make Apex feel more like one private operator John talks to?" If not, stop and correct course before building more UI, panels, review flows, duplicate agents, or visible machinery. Older phase notes are history, not permission to drift back into dashboard-first or review-first behavior.
...
- Apex OS Level 3 External Action Preparation is planned/documented locally as of 2026-06-06: `docs/APEX_OS_LEVEL_3_EXTERNAL_PREPARATION_PLAN.md` now defines preparation-only packets for order plans, booking plans, message drafts, calendar drafts, browser action plans, desktop action plans, music/second-screen plans, and deploy/production checklists. The Execution-Readiness Design Gate and Route Permission Map now require every future Level 3 packet to include exact-action preview, target/service/vendor/person/account context, data-that-would-leave summary, cost/time/location details where applicable, privacy/sensitive-data summary, risk tier, cancellation path, fallback/manual steps, receipt draft, privacy firewall recheck, prompt-injection firewall recheck, Action Permission Matrix recheck, and future Level 4 approval language. This is design/planning only: no Level 3 endpoint, UI, persistence, execution token, connector, OAuth/account connection, desktop/browser/music control, send/spend/order/book/post/calendar behavior, deploy/production/schema/auth/session/provider/billing change, deletion, or field/customer/demo Apex OS access was added.
- Apex OS Level 2 Hardening + Activity Receipts UI is complete locally as of 2026-06-06: the internal action engine now hardens alias/synonym handling for consequential work, defaults prompt-injection-looking internal writes to review/block instead of persistence, sanitizes receipt/result output with privacy redaction, and forces receipt flags so `externalActionExecuted`, `customerVisible`, and `canExecuteAfterApproval` remain false. The Control Room now has a compact operator-only `What Apex Did` / Apex Activity surface backed by existing `apexOsInternalAction` audit receipts; it shows action type, status, affected private record type/id, timestamp, short redacted reason, and undo/archive/edit/reset guidance. This added no schema change, no new external connector, no desktop/browser/music control, no send/spend/order/book/publish behavior, no auth/session/security/provider/billing change, no deploy/production/database change, no deletion path, and no field/customer/demo Apex OS access.
- Apex OS Level 2 Jarvis Internal Action Engine is complete locally as of 2026-06-06: `shared/apexOsInternalActionEngine.js` now performs the first act-by-default private/internal Apex OS actions after consulting the Action Permission Matrix, Privacy Firewall, Prompt-Injection Firewall, Tool Router/model metadata, and trace metadata. It can create/update/archive private tasks and reminders, create memory suggestions, save safe private assistant preferences, save private planning/research notes through existing memory storage, and archive private memory records, all with compact activity receipts and undo/archive/edit/reset hints. A new operator-only `POST /api/apex-os/internal-actions` endpoint persists only through existing Apex OS private company-settings stores; `runApexOsInternalAction()` was added to the API client. Ask Apex now infers only clear private internal requests such as "remind me", "create task", "remember that", and safe preference statements, and may say it saved something only when the internal action receipt reports persistence succeeded. External/consequential actions remain blocked or escalated: no email/SMS/messages, spending, orders, bookings, desktop/browser/music control, publishing, auth/session/security/provider/billing, schema/database/production/deploy, deletion, field/customer/demo exposure, secrets, or high-risk untrusted-content execution was added.
- Apex OS post-hardening checkpoint and Execution-Readiness Design Gate are complete locally as of 2026-06-06: current hardened Phase 3-10 state was checkpointed as `checkpoint/apex-os-post-hardening-hardened-state`, and `docs/APEX_OS_EXECUTION_READINESS_DESIGN_GATE.md` now defines the corrected Jarvis-style act-by-default philosophy. Apex OS should understand the request, choose the right capability/tool, act by default when the action is private, local, reversible, low-risk, and internal, report what it did, learn safely from the outcome, and interrupt John only for consequential actions. The gate defines Level 0 planning only, Level 1 draft only, Level 2 private/internal act-by-default, Level 3 external action prepared but not executed, Level 4 external action with explicit approval, Level 5 limited trusted automation, and Forbidden actions. It documents act-by-default private scope, interrupt-only scope, forbidden scope, consequential confirmation flow, exact-action preview where required, cancellation/undo, spend limits, message-send limits, booking/order rules, browser/desktop sandbox/watch mode, production/deploy hard blocks, audit metadata, privacy firewall, prompt-injection firewall, human confirmation language, dry-run mode, emergency stop/kill switch, rollback expectations, and blockers before Level 2 and external live execution. This is design/planning only: no execution, endpoint routes, UI activation, connectors, desktop/browser/music control, sending, spending, ordering, booking, production/deploy, schema/auth/session, or customer-visible behavior was added.
- Apex OS Phase 10 Apex HQ Builder/Operator Agent is complete locally as of 2026-06-06: Apex OS now has deterministic, private, source-backed, non-executing builder/operator planning helpers in `shared/apexOsBuilderOperator.js` with focused tests in `shared/apexOsBuilderOperator.test.js`. The layer classifies Apex HQ build phase planning, bug triage, code review, implementation work packages, QA validation, release readiness, schema/auth safety, production/deploy requests, customer-visible safety, and multi-workstream builder/operator intent; it returns risk tier, surface, workstream tags, plan state, required preconditions, generic plan step IDs, blocked action IDs, safe alternatives, and content-free trace metadata. Ask Apex now includes compact `builderOperatorSummary` context and the existing operator-only `/api/apex-os/ask` response exposes only that compact summary, not raw work requests, source contents, private repo data, `.env` values, secrets, credentials, production data, or an execution handle. The Personal Skill Registry now includes a planned `apex-hq-builder-operator-agent` capability that remains non-executable. Phase 10 forces `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `sourceBackedRequired: true`, `agentExecutionEnabled: false`, `codeEditEnabled: false`, `fileWriteEnabled: false`, `testRunEnabled: false`, `buildCommandEnabled: false`, `gitOperationEnabled: false`, `browserQaEnabled: false`, `deployEnabled: false`, `productionMutationEnabled: false`, `schemaAuthChangeEnabled: false`, `customerVisibleChangeEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. Privacy-blocked content, high-risk untrusted content, hidden builder/operator control, approval-gate bypass, secret exposure, field/customer/demo exposure, code/test/git execution, browser/desktop QA, deploys, production mutation, schema/auth/session changes, customer-visible changes, sends, spending, ordering, booking, calendar writes, and connector/plugin/tool/webhook runs block or approval-gate the plan instead of starting execution. No agent/subagent execution, code edit, file write, test/build command, git operation, browser/desktop/music control, deploy, production mutation, schema/auth/session/provider/permission change, endpoint, API client helper, UI activation, persistence system, connector/plugin/tool execution, send/spend/order/book/calendar behavior, hidden surveillance, or field/customer/demo Apex OS access was added. The private Jarvis control-plane roadmap through Phase 10 is now locally complete; next safe work is checkpoint/review or a new explicitly approved phase.
- Apex OS Phase 9 Life Automation Connectors is complete locally as of 2026-06-06: Apex OS now has deterministic, private, non-executing connector-readiness planning helpers in `shared/apexOsLifeAutomationConnectors.js` with focused tests in `shared/apexOsLifeAutomationConnectors.test.js`. The layer classifies connector readiness, account connection, ordering, booking, messaging, email, calendar, payment, document, and multi-connector intent, risk tier, connector surface, connector types, plan state, required preconditions, generic plan step IDs, blocked action IDs, safe alternatives, and content-free trace metadata. Ask Apex now includes compact `lifeAutomationConnectorSummary` context and the existing operator-only `/api/apex-os/ask` response exposes only that compact summary, not raw connector instructions, account details, private inbox/calendar/drive data, cookies, credentials, OAuth tokens, sessions, private logistics details, or an execution handle. The Personal Skill Registry now includes a planned `life-automation-connectors` capability that remains non-executable. Phase 9 forces `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `canConnectNow: false`, `connectorExecutionEnabled: false`, `accountConnectionEnabled: false`, `oauthFlowEnabled: false`, `credentialStorageEnabled: false`, `messageSendEnabled: false`, `emailSendEnabled: false`, `calendarWriteEnabled: false`, `orderingEnabled: false`, `bookingEnabled: false`, `paymentEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. Privacy-blocked content, high-risk untrusted content, hidden connector access, credential/session/token use, account connections, connector runs, private account reads, sends, spending, orders, bookings, calendar writes, plugin/webhook installs, and production/deploy work block or approval-gate the plan instead of starting execution. No account connection, OAuth flow, connector run, private data read, email/SMS/message/call/notification send, order, purchase, payment, booking, reservation, calendar write, plugin/webhook install, endpoint, API client helper, UI activation, persistence system, schema/auth/session change, production mutation, deploy, hidden surveillance, or field/customer/demo Apex OS access was added. Phase 10 has since been completed locally as deterministic, non-executing Apex HQ Builder/Operator Agent planning helpers with no agent execution, code edits, test runs, git operations, deploys, production mutation, schema/auth changes, or customer-visible changes.
- Apex OS Phase 8 Music + Second Screen is complete locally as of 2026-06-06: Apex OS now has deterministic, private, non-executing focus-music and second-screen setup planning helpers in `shared/apexOsMusicSecondScreen.js` with focused tests in `shared/apexOsMusicSecondScreen.test.js`. The layer classifies focus-music, playlist suggestion, music-control, audio-device, second-screen layout, dashboard display, move-window, and combined environment intent, risk tier, surface type, plan state, required preconditions, generic plan step IDs, blocked action IDs, safe alternatives, and content-free trace metadata. Ask Apex now includes compact `musicSecondScreenSummary` context and the existing operator-only `/api/apex-os/ask` response exposes only that compact summary, not raw music/app/window instructions, device state, playback history, screen layout content, cookies, credentials, account sessions, or an execution handle. Phase 8 forces `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `musicControlEnabled: false`, `audioDeviceControlEnabled: false`, `desktopWindowControlEnabled: false`, `secondScreenControlEnabled: false`, `browserControlEnabled: false`, `accountSessionUseEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. Privacy-blocked content, high-risk untrusted content, hidden control, credential/session use, music playback, device/volume changes, account work, subscriptions/spending, and desktop/window/screen control block or approval-gate the plan instead of starting execution. No music playback, audio device change, desktop/window movement, second-screen control, browser control, connector/plugin/tool execution, endpoint, API client helper, UI activation, persistence system, schema/auth/session change, production mutation, deploy, send/spend/order/book/calendar behavior, notification, hidden surveillance, or field/customer/demo Apex OS access was added. Phase 9 has since been completed locally as deterministic, non-executing Life Automation Connectors planning helpers with no account connection, OAuth, connector execution, sends, spending, ordering, booking, or calendar writes.
- Apex OS Phase 7B Browser Action Planning is complete locally as of 2026-06-06: Apex OS now has deterministic, private, non-executing browser action dry-run helpers in `shared/apexOsBrowserActionPlan.js` with focused tests in `shared/apexOsBrowserActionPlan.test.js`. The layer classifies browser/search/page/account/form/download/upload/message/money/production intent, risk tier, target type, plan state, required preconditions, generic dry-run step IDs, blocked action IDs, safe alternatives, and content-free trace metadata. Ask Apex now includes compact `browserActionSummary` context and the existing operator-only `/api/apex-os/ask` response exposes only that compact summary, not raw browser instructions, DOM, page text, cookies, credentials, or an execution handle. Phase 7B forces `operatorOnly: true`, `planningOnly: true`, `reviewFirst: true`, `browserControlEnabled: false`, `browserNavigationEnabled: false`, `clickTypeSubmitEnabled: false`, `authenticatedSessionUseEnabled: false`, `pageScrapingEnabled: false`, `downloadUploadEnabled: false`, `extensionInstallEnabled: false`, `sessionCookieUseEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. Privacy-blocked content, high-risk untrusted content, credential extraction, MFA/CAPTCHA/paywall/approval bypass, and external-account/send/spend/order/book/production requests block or approval-gate the dry-run instead of starting browser work. No browser navigation, click/type/submit, login, authenticated session use, scraping, download/upload, connector/plugin/tool execution, endpoint, API client helper, UI activation, persistence system, schema/auth/session change, production mutation, deploy, send/spend/order/book/calendar behavior, notification, hidden surveillance, or field/customer/demo Apex OS access was added. Phase 8 has since been completed locally as deterministic, non-executing Music + Second Screen planning helpers with no music/device/window/screen execution.
- Apex OS Phase 7A Desktop Sandbox / Watch Mode is complete locally as of 2026-06-06: Apex OS now has deterministic, private, non-executing desktop/browser/screen watch planning helpers in `shared/apexOsDesktopWatch.js` with focused tests in `shared/apexOsDesktopWatch.test.js`. The layer classifies explicit watch/session requests, primary surface, risk tier, blocked state, required operator controls, allowed inputs/outputs, forbidden inputs/actions, and content-free trace metadata. Ask Apex now includes compact `desktopWatchSummary` context and the existing operator-only `/api/apex-os/ask` response exposes only that compact summary, not screen content or an execution handle. Phase 7A forces `operatorOnly: true`, `manualSessionOnly: true`, `requiresExplicitStart: true`, `watchModeEnabled: false`, `desktopControlEnabled: false`, `browserControlEnabled: false`, `keyboardMouseControlEnabled: false`, `authenticatedSessionUseEnabled: false`, `screenCaptureEnabled: false`, `screenshotPersistenceEnabled: false`, `hiddenSurveillanceEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. Hidden watching, credential capture, privacy-blocked content, high-risk untrusted content, and click/type/navigation/external-account actions block or approval-gate the plan instead of starting work. No screen capture, recording, browser/desktop control, click/type/navigation, authenticated browser use, scraping, download/upload, connector/plugin/tool execution, endpoint, API client helper, UI activation, persistence system, schema/auth/session change, production mutation, deploy, send/spend/order/book/calendar behavior, notification, hidden surveillance, or field/customer/demo Apex OS access was added. Phase 7B has since been completed locally as deterministic, non-executing Browser Action Planning helpers with no navigation/click/type/login/form submission or authenticated session use.
- Apex OS Phase 7 Desktop/Browser Agent Plan is complete locally as documentation/planning only as of 2026-06-06. This phase documents the future private operator architecture for explicit-session desktop watch mode and browser action planning without adding any product execution path. The plan defines future allowed planning outputs, risk tiers, consent/session boundaries, privacy and untrusted-content gates for screen/browser/DOM content, safe trace/log metadata, cancel/pause behavior, approval packet needs, forbidden actions, and the split between Phase 7A Desktop Sandbox / Watch Mode and Phase 7B Browser Action Planning. Phase 7 keeps `operatorOnly: true`, `planningOnly: true`, `manualSessionOnly: true`, `watchModeEnabled: false`, `browserControlEnabled: false`, `desktopControlEnabled: false`, `authenticatedSessionUseEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. No desktop/browser control, click/type/navigation, authenticated browser use, screen scraping, hidden recording, screenshot persistence, connector/plugin/tool execution, shell/code execution, file download/upload, endpoint, API client helper, UI, persistence system, schema/auth/session change, production mutation, deploy, send/spend/order/book/calendar behavior, notification, approval-gate weakening, or field/customer/demo Apex OS access was added. Phase 7A has since been completed locally as deterministic, non-executing Desktop Sandbox / Watch Mode planning helpers with no keyboard/mouse/browser control and no hidden surveillance.
- Apex OS Phase 6C Knowledge Engine / Research Memory is complete locally as of 2026-06-06: Apex OS now has deterministic, private, source-aware, review-first research/knowledge helpers in `shared/apexOsKnowledgeIntelligence.js` with focused tests in `shared/apexOsKnowledgeIntelligence.test.js`. The layer ranks reviewed Apex OS knowledge sources, detects current/latest fact freshness needs, plans live research as disabled future work, builds suggested research-memory drafts for John/operator review only, and keeps compact non-executing `knowledge-summary` trace metadata without raw prompts, raw responses, private messages, document bodies, secrets, cookies, tokens, credentials, API keys, database URLs, or conversation dumps. Ask Apex now includes compact `knowledgeEngineSummary` context and source labels through the existing operator-only `/api/apex-os/ask` route; server responses expose only the compact summary, not draft bodies or execution handles. Phase 6C forces `operatorOnly: true`, `reviewFirst: true`, `sourceAware: true`, `liveWebResearchEnabled: false`, `connectorExecutionEnabled: false`, `fileSystemCrawlingEnabled: false`, `externalResearchActionsEnabled: false`, `persistenceEnabled: false`, `canExecuteNow: false`, and `executionLocked: true`. Privacy-blocked content, high-risk untrusted content, and external/high-risk approval-boundary requests block research-memory planning instead of starting work. No live web browsing, connector/plugin/tool execution, file-system crawling, automatic document/email ingestion, endpoint, API client helper, UI, persistence system, schema/auth/session change, production mutation, deploy, browser/desktop/music control, order/booking/send/spend/calendar behavior, notification, hidden surveillance, emotional profiling, or field/customer/demo Apex OS access was added. Phase 7 has since been completed locally as a documentation/planning-only Desktop/Browser Agent Plan with no desktop/browser control or external execution.
- Apex OS Phase 6B Active Intelligence Loops is complete locally as of 2026-06-06: Apex OS now has deterministic, private, manual/review-first active-intelligence loop planning helpers in `shared/apexOsActiveIntelligenceLoops.js` with focused tests in `shared/apexOsActiveIntelligenceLoops.test.js`. Loop specs cover morning planning, evening review, priority monitoring, memory suggestion review, research/watch queue, Apex HQ build progress, opportunity/risk detection, cost/token monitoring, mood/energy-aware suggestions, and what changed since last time. Ask Apex now includes compact `activeIntelligenceLoopSummary` in the existing operator-only context and content-free `background-loop-planned` trace metadata; server Ask responses expose only the compact summary through the existing operator-only `/api/apex-os/ask` response context. Loop specs and summaries force `operatorOnly: true`, `manualReviewFirst: true`, `requiresOperatorReview: true`, `triggersEnabled: false`, `backgroundExecutionEnabled: false`, `schedulerCreated: false`, `canExecuteNow: false`, and `executionLocked: true`. Privacy-blocked content, high-risk untrusted content, and approval-gated external requests block loop planning instead of starting work. No scheduler, timer, cron job, interval, recurring automation, queue, background worker, connector, endpoint, API client helper, UI, persistence system, schema/auth/session change, production mutation, deploy, browser/desktop/music control, order/booking/send/spend/calendar behavior, notification, tool/plugin/code execution, hidden surveillance, emotional profiling, or field/customer/demo Apex OS access was added. Phase 6C has since been completed locally as deterministic, private, source-aware, review-first Knowledge Engine / Research Memory helpers with no live web browsing or external execution.
- Apex OS Phase 6A Background Thinking Scheduler Plan is complete locally as of 2026-06-06 as documentation/planning only. This phase defines the future private active-intelligence scheduler as an operator-only planning layer, not an agent runner, execution engine, recurring automation, hidden watcher, connector, desktop/browser/music controller, purchasing/booking/messaging tool, deployment tool, or surveillance system. No timer, cron job, interval, recurring automation, queue, background worker, endpoint, API client helper, UI, persistence system, schema/auth/session change, production mutation, deploy, connector, browser/desktop/music control, order/booking/message/calendar/spend behavior, notification, tool/plugin/code execution, or field/customer/demo Apex OS access was added. The plan requires every future loop to have a written spec with trigger type/source, allowed inputs, privacy/untrusted-content gates, model route/tier, token/time budget, cooldown, daily cap, output type, approval needs, safe trace metadata, pause/cancel behavior, forbidden actions, `operatorOnly: true`, `canExecuteNow: false`, and `executionLocked: true`. All future triggers stay disabled by default; future loop states stay review-first; outputs are limited to private briefs/digests, suggested memory/task/reminder drafts, research-note drafts, approval-packet drafts, next-safe-action recommendations, or blocked-state explanations. Phase 6B has since been completed locally as deterministic, private, manual/review-first active-intelligence loop planning helpers with no unmanaged background execution and no external actions.
- Apex OS Phase 6 Affective State is complete locally as of 2026-06-06: Apex OS now has a deterministic, private, non-executing affective-state helper in `shared/apexOsAffectiveState.js` with compact mode, tone, urgency, energy, frustration, focus, and response-style classification. It detects conversation cues such as concise/step-by-step/explainer style requests, urgency, frustration, overwhelm, low/high energy, focus, confusion/stuckness, exploratory mode, execution readiness, and recovery/reset language, then returns safe guidance for response adaptation only. Ask Apex now includes `affectiveStateSummary` in the existing operator-only context and content-free `affective-state` trace metadata; local fallback can adapt response style for frustrated/overloaded/stuck/urgent turns. Server Ask responses expose only the compact summary through the existing operator-only `/api/apex-os/ask` response context. The layer is not diagnosis, not clinical inference, not a durable psychological profile, not stored as memory, and not executable; it forces `canExecuteNow: false`, `operatorOnly: true`, `storesRawText: false`, `storesPsychProfile: false`, and `safeToStoreDurably: false`. No endpoint, API client helper, UI, persistence, schema/auth/session change, provider credential change, production mutation, deploy, external action, desktop/browser/music/order/send/spend behavior, notification, or field/customer/demo Apex OS access was added. Phase 6A has since been completed locally as a documentation/planning-only scheduler safety plan with no background execution.
- Apex OS Phase 5C Untrusted Content / Prompt-Injection Firewall is complete locally as of 2026-06-06: Apex OS now has a deterministic, non-executing prompt-injection boundary in `shared/apexOsUntrustedContentFirewall.js` with trust levels (`trusted-operator`, `trusted-internal`, `trusted-project-doc`, `untrusted-web`, `untrusted-browser`, `untrusted-email`, `untrusted-document`, `untrusted-file`, `untrusted-tool-output`, `untrusted-user-paste`, `unknown`), source types (`web-page`, `search-result`, `browser-dom`, `email-body`, `document-text`, `file-content`, `clipboard-paste`, `tool-output`, `external-api`, `unknown`), and risk levels (`none`, `low`, `medium`, `high`, `critical`). The firewall detects prompt-injection patterns such as ignore previous/system/developer instructions, prompt/hidden-instruction reveal attempts, secret exfiltration, send/click/download/run/install/delete/permission-change instructions, approval bypass, conceal-from-operator instructions, role impersonation, instruction overrides, and deterministic encoded/obfuscated variants. It returns sanitized text, stripped instruction IDs, compact detected pattern IDs, safe summaries, and content-free metadata only. Ask Apex treats John's direct question as trusted operator input while filtering pasted/live context as untrusted data, includes `untrustedContentFirewallSummary`, and falls back locally when untrusted source context requires review. Knowledge Intelligence sanitizes suggested/document-like source summaries before provider packets and keeps high/critical untrusted rows local. The Tool Router blocks high/critical untrusted context before route planning, and External Action Approval drafts carry untrusted-source warnings without execution. Trace metadata now includes a content-free `untrusted-content-firewall` event/source. No endpoint, API client helper, UI, persistence, schema/auth/session change, provider credential change, production mutation, deploy, tool/skill execution, plugin execution, desktop/browser/music/ordering behavior, notification, send, spend, or field/customer/demo Apex OS access was added. Phase 6 has since been completed locally as a non-executing affective-state classification layer.
- Apex OS Phase 5B External Action Approval System is complete locally as of 2026-06-06: Apex OS now has a centralized review-only approval bridge in `shared/apexOsExternalActionApprovals.js` that converts Tool Router plans into compact approval summaries and route-linked approval packet drafts without execution. Phase 5B defines external approval statuses (`not-required`, `draft-available`, `future-tool-planned`, `blocked`, `forbidden`, `unavailable`), scopes (`internal-only`, `external-action`, `high-risk`, `future-tool`, `blocked`), category mapping for ordering, booking, messaging, email, calendar, deployment, production, schema/auth/session, billing/payment, file-write, browser/desktop, and music, exact approval phrases, required evidence lists, safe trace metadata, and a no-execution guard that forces `canExecuteNow: false`, `canExecuteAfterApproval: false`, and `executionLocked: true`. Existing approval packets now support Phase 5B route metadata (`sourceRouteId`, `sourceRouteStatus`, `sourceActionDomain`, `approvalSystemPhase`, `executionGate`) and new categories (`email`, `messaging`, `calendar`, `ordering`, `booking`, `file-write`, `browser-desktop`, `music`, `external-action`) while still rejecting execution statuses and unsafe secret/email content. Ask Apex now includes `externalActionApprovalSummary`, adds safe approval-gate trace metadata, and its existing "Needs approval" draft flow can create route-aware approval packets through the existing operator-only approval-packets endpoint. Existing Control Room approval packet category options were extended; no new execute endpoint, approval-submit endpoint, connector route, external action route, schema/auth/session change, production mutation, deploy, desktop/browser/music/ordering execution, notification, send, spend, or field/customer/demo Apex OS access was added. Approved packets remain records only and do not execute or unlock tools. Phase 5C has since been completed locally as a non-executing untrusted-content firewall.
- Apex OS Phase 5 Tool Router is complete locally as of 2026-06-06: Apex OS now has a centralized, deterministic, non-executing routing/design helper in `shared/apexOsToolRouter.js` with route constants (`answer-only`, `memory-read`, `memory-suggest`, `memory-review`, `task-reminder-read`, `task-reminder-write`, `planning`, `research-plan`, `knowledge-summary`, `apex-hq-build-help`, `file-read-plan`, `file-write-plan`, `browser-plan`, `desktop-plan`, `music-plan`, `ordering-plan`, `booking-plan`, `messaging-plan`, `email-plan`, `calendar-plan`, `deployment-plan`, `production-plan`, `blocked`), route statuses (`available-non-executing`, `planned`, `approval-required`, `blocked`, `forbidden`, `unavailable`), route categories (`answer`, `memory`, `tasks-reminders`, `planning`, `research`, `knowledge`, `apex-hq`, `files`, `browser`, `desktop`, `music`, `ordering`, `booking`, `communication`, `calendar`, `deployment`, `production`, `safety`, `system`), input normalization, route planning, route safety evaluation, compact summary, and a no-execution guard. The router consumes or accepts Personal Skill Registry, Action Permission Matrix, Model Router + Cost Governor, Trace + Learning Log, and Privacy Firewall outputs; all route plans force `canExecuteNow: false` and `executionLocked: true`. Ask Apex now includes a compact `toolRouteSummary` and packet-local safe route trace metadata so it can explain whether Apex can answer, draft, plan, require approval, report a planned/unavailable tool, or block a request without claiming execution. No endpoint, API client helper, UI, persistence, schema/auth/session change, provider credential change, production mutation, deploy, tool execution, skill execution, desktop/browser/music/ordering behavior, notification, send, spend, or field/customer/demo Apex OS access was added. Phase 5B has since been completed locally as a review-only External Action Approval System.
- Apex OS Phase 4.7 Redaction-Before-Cloud / Privacy Firewall is complete locally as of 2026-06-06: Apex OS now has a centralized, deterministic, non-executing privacy firewall in `shared/apexOsPrivacyFirewall.js` with sensitivity categories (`secret`, `credential`, `api-key`, `token`, `cookie`, `authorization-header`, `db-url`, `payment`, `ssn`, `phone`, `email`, `address`, `private-personal`, `medical`, `legal`, `financial`, `customer-data`, `company-private`, `field-restricted`, `production-data`, `unknown-sensitive`), privacy actions (`allow`, `redact`, `summarize-only`, `approval-required`, `block`), trusted/untrusted context constants, detection helpers, redaction helpers, privacy classification, cloud-send decisions, safe metadata, compact summaries, and recursive payload sanitization. Secrets, credentials, tokens, cookies, authorization headers, API keys, database URLs, and inherited redaction artifacts are blocked or redacted before cloud/external contexts; phone/email/address are redacted by default for cloud/external contexts; payment/SSN/unknown-sensitive content requires approval; private personal, medical, legal, financial, customer, company-private, field-restricted, and production data are summary-only or blocked depending on target context; field/customer/demo targets are blocked from Apex OS private/operator content. Ask Apex now keeps a compact `privacyFirewallSummary`, uses sanitized cloud-bound context, redacts sensitive local echo text, and falls back locally instead of calling the provider when the firewall blocks or requires approval. Knowledge Intelligence now adds `privacyFirewallSummary`, sanitizes provider summary rows/warnings, and stays local when firewall blocks or requires approval. No endpoint, API client helper, UI, persistence, schema/auth/session change, provider credential change, production mutation, deploy, tool/skill execution, desktop/browser/music/ordering behavior, notification, send, spend, or field/customer/demo Apex OS access was added. Focused privacy/Ask/Knowledge/Trace validation passed. Phase 5 has since been completed locally as a non-executing Tool Router.
- Apex OS Phase 4.6 Trace + Learning Log is complete locally as of 2026-06-06: Apex OS now has a centralized, non-executing, metadata-only trace helper in `shared/apexOsTraceLog.js` with event type constants (`ask-request`, `model-route`, `skill-registry-context`, `action-permission-classification`, `memory-suggestion`, `memory-review`, `task-reminder-context`, `knowledge-summary`, `background-loop-planned`, `approval-required`, `forbidden-action`, `error`), status constants (`started`, `completed`, `skipped`, `blocked`, `approval-required`, `forbidden`, `error`), source constants, normalization helpers, trace entry creation, unsafe-field detection/collection, stripping/rejection helpers, credential-like value redaction, compact summaries, and bounded pruning. Ask Apex now attaches compact trace metadata for request context, model route, action permission, skill registry, task/reminder context, memory-suggestion status, and approval/forbidden gates; Knowledge Intelligence attaches compact safe-summary route/status trace metadata. Trace entries force `canExecuteNow: false` and do not store raw prompts, raw responses, messages, private message bodies, document bodies, headers, cookies, tokens, credentials, API keys, database URLs, or conversation dumps. Persistence, `GET /api/apex-os/traces`, API client helper, and UI were intentionally skipped to avoid creating hidden surveillance or broad storage; traces are currently packet-local metadata returned only through existing operator-only Apex OS contexts. No provider credentials, billing settings, schema/auth/session behavior, production data, deploys, tool execution, skill execution, desktop/browser/music/ordering execution, notifications, messages, spending, external actions, or customer/field/demo Apex OS access was added. Focused Phase 4.6 tests passed for trace helpers, Ask Apex, and Knowledge Intelligence; broader validation is tracked in the implementation report. Phase 4.7 has since been completed locally as a non-executing privacy firewall helper phase.
- Apex OS Phase 4.5 Model Router + Cost Governor is complete locally as of 2026-06-06: Apex OS now has a centralized non-executing model routing and budget metadata module in `shared/apexOsModelRouter.js` with model tiers (`nano`, `mini`, `standard`, `flagship`), routes (`intent-classification`, `memory-suggestion`, `task-summary`, `safe-summary`, `normal-chat`, `planning`, `research`, `knowledge-synthesis`, `tool-routing`, `permission-classification`, `complex-reasoning`, `coding-analysis`, `risk-review`, `affective-state`, `background-loop`), budget levels (`tiny`, `small`, `normal`, `deep`), route-to-tier/model-alias selection, max output token caps, escalation policy, safe unknown-route fallback, model alias override support, and safe usage metadata that does not store raw prompts, responses, messages, cookies, tokens, credentials, or private content. Current audit found Apex OS provider usage in Ask Apex (`shared/apexOsAsk.js`), Knowledge Intelligence (`shared/apexOsKnowledgeIntelligence.js`), and Voice (`shared/apexOsVoice.js`), plus older contractor AI helpers using `gpt-4o-mini`; this phase safely wired Ask Apex and Knowledge Intelligence only, leaving voice/audio model IDs and older contractor AI helpers unchanged. Ask Apex now infers a compact internal model route from assistant mode, action risk, and request type, applies a `max_tokens` cap, and returns compact `modelRoutingSummary` in the existing operator-only Ask response context; Knowledge Intelligence now gets its existing cheap summary model alias and compact cap from the central router. No provider credentials, billing settings, schema/auth/session behavior, production data, deploys, tool execution, skill execution, desktop/browser/music/ordering execution, notifications, messages, spending, new endpoint, model-switch UI, or customer/field/demo Apex OS access was added. Validation passed: focused model router, Ask Apex, and Knowledge Intelligence tests; broader Phase 1/2/3/4/4A Apex OS regression tests; and `npm.cmd run build` with the existing large-chunk warning only. Phase 4.6 has since been completed locally as a metadata-only Trace + Learning Log module.
- Apex OS Phase 4A Action Permission Matrix is complete locally as of 2026-06-06: Apex OS now has a centralized deterministic, operator-only purpose, non-executing risk classifier in `shared/apexOsActionPermissions.js` with risk tiers (`safe-answer`, `safe-read`, `internal-write`, `approval-required`, `external-action`, `high-risk`, `forbidden`), action domains (`conversation`, `memory`, `tasks`, `research`, `planning`, `apex-hq`, `files`, `desktop`, `browser`, `music`, `ordering`, `booking`, `messaging`, `email`, `calendar`, `billing`, `auth`, `schema`, `production`, `deployment`, `system`), approval requirement constants, forbidden action constants, normalization helpers, classifier helpers, allowed/approval/forbidden helpers, and a compact Ask Apex summary helper. `canExecuteNow` is forced false for every classification in Phase 4A. Safe answer/read/planning work can be described privately; memory/task/reminder/planning writes are classified as internal-write only through existing operator-only endpoints; money/order/booking/message/email/calendar/browser/desktop/music/file-write actions are approval-gated; auth/schema/production/deploy/delete actions are high-risk and approval-packet gated; hidden GPS/location tracking, secret exposure or raw credential memory storage, approval/permission weakening, field/customer/demo Apex OS access, automatic sends/spend, approval bypass, and contractor-facing Apex OS without an explicit future phase are forbidden. Ask Apex now receives compact action-permission context and returns the compact summary in the existing operator-only Ask response context; no execute endpoint, new approval endpoint, tool router, API client helper, or UI surface was added. Validation passed: focused action-permission and Ask Apex tests, broader assistant modes, tasks/reminders, memory, skill registry, permissions, server Apex OS memory endpoint, and API client tests; `npm.cmd run build` passed with the existing large-chunk warning only. No `.env` was opened, no secrets were exposed, no schema/auth/session/provider/production/deploy change was made, no field/customer/demo access was added, and Phase 4.5 Model Router was not started.
- Apex OS Phase 4 Personal Skill Registry is complete locally as of 2026-06-06: Apex OS now has a centralized private operator-only, non-executing skill/capability registry in `shared/apexOsSkillRegistry.js` with status constants (`available`, `planned`, `disabled`, `blocked`, `deprecated`), category constants (`memory`, `planning`, `knowledge`, `apex-hq`, `life`, `automation`, `communication`, `environment`, `safety`, `system`), risk constants (`safe-read`, `internal-write`, `approval-required`, `external-action`, `forbidden`), normalization helpers, default registry builder, filtering helpers, available/planned helpers, executable checks, and a compact Ask Apex summary helper. All Phase 4 skills are forced `operatorOnly: true` and `canExecute: false`; planned/disabled/blocked skills cannot be treated as executable. A read-only operator-only `GET /api/apex-os/skills` endpoint was added, along with `getApexOsSkills(token)` in `src/api.js`; no execute, plugin, tool-router, desktop/browser/music/ordering/booking/message/send/spend/deploy/schema/auth/production endpoint was added. Ask Apex now receives compact skill context so it can honestly say what is available, planned, disabled, or blocked without dumping the full registry or claiming execution. Minimal UI was intentionally skipped for Phase 4 because the Control Room is already a large dirty UI surface and the registry goal is complete through shared/server/API/Ask context; a compact Capabilities panel can be a future Phase 4B UI slice if John wants it. Validation passed: focused skill registry, Ask Apex, API client, server/operator-only endpoint, assistant modes, tasks/reminders, memory, permissions tests; `npm.cmd run build` passed with the existing large-chunk warning only. No `.env` was opened, no secrets were exposed, no schema/auth/session/provider/production/deploy change was made, no field/customer/demo access was added, and Phase 4A was not started.
- Apex OS Phase 3D Active Intelligence + Knowledge Engine Plan is complete locally as a documentation/planning pass as of 2026-06-06. It documents how Apex OS should become proactive and source-aware without trying to store all world knowledge locally: Apex should know what it remembers, know what app/project/docs/files/tasks/tool results it can access, know when to research live information, know when to use a stronger model, save useful learnings through reviewed suggestions, and avoid endless token-burning loops. The plan defines safe background loops for morning planning, evening review, priority monitoring, memory suggestion review, research/watch queues, Apex HQ build progress tracking, opportunity/risk detection, cost/token monitoring, mood/energy-aware suggestions, and "what changed since last time?" checks. It also defines the future Knowledge Engine for live web research, source-aware answers, saved research notes, personal/project knowledge bases, document/file/memory/task retrieval, tool-result summaries, compact prompt summaries, freshness checks, and trusted-vs-untrusted content handling. OpenJarvis lessons were added as architecture inspiration only; no dependency was added, nothing was installed, no schema/auth/session/provider/production/deploy change was made, no browser/desktop/music/food-ordering execution was added, no messages were sent, no money was spent, and Phase 4 was not started.
- Apex OS Phase 3B Memory Suggestions Review UX is complete and checkpointed locally as of 2026-06-06. Checkpoint stash: `checkpoint/apex-os-memory-suggestions-phase-3b-complete`, currently listed as `stash@{0}` when it was verified. The checkpoint includes `src/apex-control-room-components-import.test.js`, `src/apex-control-room-components.jsx`, `src/apex-control-room-utils.js`, and `src/apex-control-room-utils.test.js`. Phase 3B added a dedicated operator-only Memory Suggestions review area, kept suggested and approved memories separate, preserved the existing memory API boundaries, and did not start Phase 4. Validation passed before checkpointing: focused Phase 3B tests, broader Phase 1/2/3 regression coverage, `npm.cmd run build` with the existing large-chunk warning only, `git diff --check` with CRLF warnings only, and real desktop/mobile/empty/suggested/approve/archive visual QA. Apex OS stayed operator-only; field/customer/demo users did not gain access.
- Apex OS Phase 3C Private Life Operator Architecture is complete locally as a documentation/planning pass as of 2026-06-06. This pass records that Apex OS is John's private Jarvis-style life operator, not only an Apex HQ contractor-app assistant and not currently a customer-facing contractor AI feature. Apex HQ is one important workspace/domain inside Apex OS. Phase 3C documents personal memory, life routines, goals, preferences, tasks/reminders, affective state, active thinking loops, research/knowledge, future desktop/browser/app control, music/second-screen planning, approval-gated ordering/booking/personal logistics, and the Apex HQ builder/operator workspace. This pass is docs-only: no schema, auth/session, provider, production-data, deploy, browser/desktop/music/food-ordering execution, messages, spending, customer-visible behavior, permission loosening, or Phase 4 Skill Registry work is approved or added.
- Apex Live Operator Mode native state-animation pass is complete locally as of 2026-06-06: the approved Apex intelligence GLB now exports seven native animation clips from Blender: `Idle`, `Listening`, `Hearing`, `Thinking`, `Speaking`, `Blocked`, and `Alert`. The desktop and mobile optimized runtime GLBs preserve those clips with no required glTF extensions, and the Apex cockpit/Avatar Lab now crossfade to the matching native clip instead of playing every imported animation at full weight. The live voice/body QA now captures the active state clip marker, proving the page selected `Hearing` on the desktop post-answer handoff and `Speaking` on the mobile talkback turn. No schema, auth/session, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible action paths were added. Local validation passed: focused Apex Control Room/avatar-lab import tests, GLB inspection for both optimized runtime assets with all seven clips and no required extensions, `npm.cmd run build` with the existing large-chunk warning only, `npm.cmd run verify:roles`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile live voice/body QA against a disposable SQLite demo-operator workspace. Evidence: `outputs/apex-live-voice-body-qa/2026-06-06T04-24-43-301Z/desktop-voice-turn-talkback.png`, `outputs/apex-live-voice-body-qa/2026-06-06T04-24-43-301Z/mobile-voice-turn-talkback.png`, and the related QA output directory. This pass has not been production deployed yet in this turn.
- Apex Live Operator Mode GLB runtime avatar pass is complete locally as of 2026-06-06: the private Apex cockpit now uses the original Meshy/Blender Apex intelligence GLB as the live body instead of relying on the procedural/static body. The 32.6 MB app-ready master remains in `public/assets/apex-avatar/apex-intelligence-idle-app-ready.glb`, while optimized standard GLB runtime assets were added for desktop (`apex-intelligence-live-web-1536.glb`, 10.9 MB) and mobile (`apex-intelligence-live-web-1024.glb`, 7.6 MB). The cockpit selects the mobile runtime model on small viewports, preserves the master and procedural GLB fallback path, and increases state-reactive hover, awareness-line, core, ring, light, and speech-pulse motion so the body feels more like an active Apex intelligence. The avatar lab now points at the optimized runtime asset while keeping the master fallback. The repeatable live voice/body QA now asserts the GLB renderer, `apex-intelligence-live-web` model marker, and live canvas on desktop and mobile. No schema, auth/session, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible action paths were added. Local validation passed: focused Apex Control Room/avatar-lab import tests, GLB inspection for both optimized runtime assets with `Idle` animation preserved and no required extensions, `npm.cmd run build` with the existing large-chunk warning only, `npm.cmd run verify:roles`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile live voice/body QA against a disposable SQLite demo-operator workspace. Evidence: `outputs/apex-live-voice-body-qa/2026-06-06T04-05-58-066Z/desktop-voice-turn-talkback.png`, `outputs/apex-live-voice-body-qa/2026-06-06T04-05-58-066Z/mobile-voice-turn-talkback.png`, and the related QA output directory. This pass has not been production deployed yet in this turn.
- Apex Live Operator Mode live voice/body pass is complete locally as of 2026-06-05: the Apex cockpit now has a desktop `Sound Check` that audibly says "Apex audio is on. I can talk back on this desktop.", tracks speaker readiness as visible voice health, recovers quickly if browser speech does not finish cleanly, uses browser voice fallback when server speech is not configured, and preserves the open voice loop so Apex can hear a spoken request, speak the answer back, then reopen live listening. The Apex body is no longer just the flat source image: the cockpit overlays state-reactive aura, neural-grid, and voice-band animation layers on top of the body, with speaking/hearing/listening visual states verified on desktop. A repeatable local QA runner was added at `scripts/apex-live-voice-body-qa.mjs`. This pass does not add schema, auth/session changes, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible action paths. Local validation passed: focused Apex Control Room/shared voice tests (18 passing), broader Apex Control Room/shared/server autonomy and memory suite (55 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, and Playwright desktop browser QA against a disposable SQLite demo-operator workspace. Browser QA confirmed the temp demo admin operator flag was local-only, Sound Check spoke through the browser harness, Apex heard the spoken transcript `what's blocked today`, Apex spoke a source-backed answer back, the voice loop reopened listening after the answer, body states included speaking/listening/hearing, aura/neural-grid/voice-band layers rendered, horizontal overflow was zero, and there were no console/page/request errors. Evidence: `outputs/apex-live-voice-body-qa/2026-06-05T20-36-52-951Z/summary.json`, `outputs/apex-live-voice-body-qa/2026-06-05T20-36-52-951Z/desktop-sound-check-speaking.png`, `outputs/apex-live-voice-body-qa/2026-06-05T20-36-52-951Z/desktop-voice-turn-talkback.png`, and `outputs/apex-live-voice-body-qa/2026-06-05T20-36-52-951Z/desktop-post-answer-listening.png`. This pass has not been production deployed yet in this turn.
- Apex Live Operator Mode conversation continuity production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `c9e9324` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v706`, image `registry.fly.io/concrete-ops-2:deployment-01KTCETQ2DHX0QAR3QF25QYBYS`; rollback target is `v705`, image `registry.fly.io/concrete-ops-2:deployment-01KTCCFJE18KNVTKV99VF06ZC8`. The deploy was run from a clean temporary worktree at `C:\Users\jberl\AppData\Local\Temp\apex-conversation-continuity-release-c9e9324-20260605105159` so unrelated local brief/output files were not part of the release. Predeploy checks passed from that clean release worktree: `npm.cmd ci`, focused Apex Control Room/shared/server tests (50 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright conversation-continuity QA. Production backup before deploy produced `postgres-app-data-20260605-175458Z.json`, `uploads-20260605-175458Z`, and `uploads-20260605-175458Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v706` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, both `/api/setup/status` endpoints returned setup complete with demo mode off, demo user absent, public signup disabled, and public estimate requests enabled, `/apex-control-room` served `index-Db3rs2bl.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-DZfEW1KM.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe`, and recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode conversation continuity pass is complete locally as of 2026-06-05: the body-first Apex cockpit now carries the last operator request, last Apex answer summary, matched room/action, active private run, next private move, interruption state, retry state, and recent turn history into the next answer so short follow-ups like "yes", "do it", "that", "keep going", and "yes do that next" resolve naturally instead of starting cold. The Ask Apex request now keeps the operator's visible question separate from the hidden live conversation context, so local/provider fallback answers do not echo internal prompt envelopes. The desktop detailed console and compact mobile response both expose a visible Conversation Context card with Thread/Route/Run/Move evidence. A repeatable local QA runner was added at `scripts/apex-conversation-continuity-qa.mjs`. This pass does not add schema, auth/session changes, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible action paths. Local validation passed: focused Ask Apex/Apex Control Room/API tests (21 passing), broader Apex Control Room/shared/server autonomy and memory suite (50 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile browser QA against a disposable SQLite demo-operator workspace served from the built app. Browser QA confirmed the temp demo admin operator flag was local-only, the Apex body rendered, the second short follow-up `yes do that next` appeared in visible conversation context, hidden context did not leak into fallback response text, desktop/mobile Conversation Context cards were visible, Thread/Route/Run/Move rows rendered, desktop/mobile horizontal overflow was zero, and there were no console/page/request errors. Evidence: `outputs/apex-conversation-continuity-qa/2026-06-05T17-47-49-801Z/summary.json`, `outputs/apex-conversation-continuity-qa/2026-06-05T17-47-49-801Z/desktop-conversation-continuity.png`, `outputs/apex-conversation-continuity-qa/2026-06-05T17-47-49-801Z/desktop-conversation-context.png`, `outputs/apex-conversation-continuity-qa/2026-06-05T17-47-49-801Z/mobile-conversation-continuity.png`, and `outputs/apex-conversation-continuity-qa/2026-06-05T17-47-49-801Z/mobile-conversation-context.png`.
- Apex Live Operator Mode voice retry recovery production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `0b0256a` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v705`, image `registry.fly.io/concrete-ops-2:deployment-01KTCCFJE18KNVTKV99VF06ZC8`; rollback target is `v704`, image `registry.fly.io/concrete-ops-2:deployment-01KTCAWQTC6WGBN547AHPX2143`. The deploy was run from a clean temporary worktree at `C:\Users\jberl\AppData\Local\Temp\apex-voice-retry-release-0b0256a-20260605101042` so unrelated local brief/output files were not part of the release. Predeploy checks passed from that clean release worktree: `npm.cmd ci`, focused Apex Control Room/shared/server tests (43 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check`, plus desktop/mobile Voice Retry Playwright QA from the active worktree. Production backup before deploy produced `postgres-app-data-20260605-171219Z.json`, `uploads-20260605-171219Z`, and `uploads-20260605-171219Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v705` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, both `/api/setup/status` endpoints returned setup complete with demo mode off, demo user absent, public signup disabled, and public estimate requests enabled, `/apex-control-room` served `index-2H4pDN8W.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-hLNAEPlZ.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe`, and recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode operating memory refreshed on 2026-06-05: the active thread goal remains one complete private Apex Live Operator Mode, not a random dashboard/polish loop. John approved commit, push, and production deploy after each verified slice for this goal. Codex must keep working from the repo contract and Apex skills, preserve the body-first Apex command room, validate locally with focused tests/build/desktop and mobile browser QA before release, update this living plan, then commit, push, deploy, and smoke production. External/customer-visible actions remain locked unless separately approved: live email/SMS/customer sends, ads/ad spend, billing/payment, customer-visible publishing/sharing, provider credential changes, hidden/background microphone capture, schema/auth/session changes, production data mutation outside the approved release path, deletion, deploy/rollback execution from the UI, automatic trusted memory, autonomous unrequested agent execution, and irreversible external actions.
- Apex Live Operator Mode voice retry recovery pass is complete locally as of 2026-06-05: the body-first Apex cockpit now treats missed, empty, or provider-limited voice transcription turns as explicit retry-listening recovery turns. Apex increments a visible retry lane, records a command-stream retry item, can speak "I missed that. Say it again and I'll keep listening.", reopens visible live listening after the retry handoff when the operator has already wake-granted the page, and keeps the Voice Health / live pulse surfaces showing `Retry: Ready` or retry counts. Successful follow-up transcripts update the retry reason as recovered. This pass does not add schema, auth/session changes, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible action paths. Local validation passed: focused Apex Control Room/shared/server tests (43 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile browser QA against a disposable SQLite demo-operator workspace served from the built app. Browser QA confirmed the temp demo admin operator flag was local-only, desktop/mobile Apex body rendered, Voice Health and retry lane text were visible, desktop/mobile horizontal overflow was zero, and there were no console/page/request errors. Evidence: `outputs/apex-voice-retry-qa/2026-06-05T17-07-43-538Z/summary.json`, `outputs/apex-voice-retry-qa/2026-06-05T17-07-43-538Z/desktop-voice-retry-health.png`, and `outputs/apex-voice-retry-qa/2026-06-05T17-07-43-538Z/mobile-voice-retry-health.png`.
- Apex Live Operator Mode Auto Drive operator handback pass is complete locally as of 2026-06-05: server-backed private Auto Drive now returns explicit `handbackRequired`, `stopReason`, `handbackTitle`, and `handbackRecommendation` fields at manual review gates, and the Apex body-first command room converts that stop into a full spoken operator handback with `What I Did`, `Proof I Have`, `What I Need`, and `Still Locked` sections instead of a short stop message. Auto Drive still disables at the manual review gate and keeps execution locked. No schema, auth/session, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible external action path was added. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile browser QA against a disposable SQLite demo-operator workspace. Browser QA created a temporary private run, advanced it through `draft-internal` and `private-prep` by API, clicked the real UI `Auto Drive` control for the `proof-check` review-gate stop, confirmed the full handback notice and four operator sections on desktop, confirmed browser speech captured the operator handback, confirmed mobile safety/handback evidence, found zero horizontal overflow at 1440x920 and 390x844, and found no console/page/request errors. Evidence: `outputs/apex-auto-drive-handback-qa/2026-06-05T16-37-55-135Z/summary.json`, `outputs/apex-auto-drive-handback-qa/2026-06-05T16-37-55-135Z/desktop-auto-drive-handback.png`, and `outputs/apex-auto-drive-handback-qa/2026-06-05T16-37-55-135Z/mobile-auto-drive-handback.png`.
- Apex Live Operator Mode Auto Drive operator handback production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `6dd296e` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v704`, image `registry.fly.io/concrete-ops-2:deployment-01KTCAWQTC6WGBN547AHPX2143`; rollback target is `v703`, image `registry.fly.io/concrete-ops-2:deployment-01KTC8N405F530DE8FYK2VVJND`. The deploy was run from a clean temporary worktree at `C:\Users\jberl\AppData\Local\Temp\apex-auto-drive-handback-release-6dd296e` so unrelated local brief/output files were not part of the release. Predeploy checks passed from that clean release worktree: `npm.cmd ci`, focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check`, plus desktop/mobile Auto Drive handback Playwright QA from the active worktree. Production was autostopped before release, so the machine was started and both production `/api/ready` endpoints returned ready/database ok before backup. Production backup before deploy produced `postgres-app-data-20260605-164559Z.json`, `uploads-20260605-164559Z`, and `uploads-20260605-164559Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v704` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, both `/api/setup/status` endpoints returned setup complete with demo mode off, demo user absent, public signup disabled, and public estimate requests enabled, `/apex-control-room` served `index-kCcIIPZA.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-DXFP1rQj.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe`, and recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode desktop stage-view pass is complete locally as of 2026-06-05: the Apex cockpit now defaults to a cleaner body-first `Stage View` on desktop with a compact `Apex body-first stage HUD`, `Apex stage status chips`, and `Apex stage command dock` for briefing, watch report, safe private run work, and opening the full console. The spotlight first viewport hides the crowded quick-turn buttons, command stream, and detailed Watch/Mission/Run panels while preserving the Ask bar, boundary strip, and the large Apex digital body; the `Console`/`Full Console` path restores the dense operator controls when needed. Mobile keeps its existing Apex dock and response panels instead of using the desktop stage HUD. No backend behavior, schema, auth/session, role permission, production data, voice capture, external send, billing/payment, provider action, deploy/rollback UI execution, automatic trusted memory, autonomous unrequested agent execution, deletion, or irreversible action path was added. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile browser QA against a disposable SQLite demo-operator workspace with local-only `operator_access` enabled. Browser QA confirmed the desktop stage HUD, stage dock, stage status chips, Apex body, and Apex Now shell render; the crowded desktop spotlight panels are hidden; full console restores quick turns, command stream, and Watch Officer; mobile keeps the dock/response/body; desktop/mobile had zero horizontal overflow and no console/page/request errors. Evidence: `outputs/apex-stage-view-qa/2026-06-05T16-01-22-419Z/summary.json`, `outputs/apex-stage-view-qa/2026-06-05T16-01-22-419Z/desktop-stage-view.png`, `outputs/apex-stage-view-qa/2026-06-05T16-01-22-419Z/desktop-full-console.png`, and `outputs/apex-stage-view-qa/2026-06-05T16-01-22-419Z/mobile-stage-view.png`.
- Apex Live Operator Mode desktop stage-view production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `986961d` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v703`, image `registry.fly.io/concrete-ops-2:deployment-01KTC8N405F530DE8FYK2VVJND`; rollback target is `v702`, image `registry.fly.io/concrete-ops-2:deployment-01KTC6PYFNBZT6C8K30YD70SB4`. The deploy was run from a clean temporary worktree at `C:\Users\jberl\AppData\Local\Temp\apex-stage-view-release-986961d` so unrelated local brief/output files were not part of the release. Predeploy checks passed from that clean release worktree: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check`, plus desktop/mobile Stage View Playwright QA from the active worktree. Production backup before deploy produced `postgres-app-data-20260605-160633Z.json`, `uploads-20260605-160633Z`, and `uploads-20260605-160633Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v703` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, both `/api/setup/status` endpoints returned setup complete with demo mode off, demo user absent, and public signup disabled, `/apex-control-room` served `index-nYEGUgkq.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-St2rgbXP.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe`, and recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode Closing Report production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `59075b4` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v702`, image `registry.fly.io/concrete-ops-2:deployment-01KTC6PYFNBZT6C8K30YD70SB4`; rollback target is `v701`, image `registry.fly.io/concrete-ops-2:deployment-01KTC4JP4CG46VN7FM38PEZSRD`. The deploy was run from a clean temporary worktree at `C:\Users\jberl\AppData\Local\Temp\apex-closing-report-release-59075b4` so unrelated local brief/output files were not part of the release. Predeploy checks passed from that clean release worktree: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check`, plus desktop/mobile Closing Report Playwright QA from the active worktree. Production backup before deploy produced `postgres-app-data-20260605-153309Z.json`, `uploads-20260605-153309Z`, and `uploads-20260605-153309Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v702` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, both `/api/setup/status` endpoints returned setup complete with demo mode off, demo user absent, public signup disabled, and public estimate requests enabled, `/apex-control-room` served `index-Bzztqi6S.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-UCCLFFAZ.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CyeERXNN.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe`, and recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode Closing Report pass is complete locally as of 2026-06-05: Apex now has a first-class `Closing Report` under the active private run handback in the body-first command room. It summarizes work, evidence, memory, decision, and locks from the existing server-backed autonomy-run ledger, operator handback, next private move, and run-memory review state. Natural commands such as `closing report`, `closeout report`, `final report`, `operator closing`, `what did you finish`, and `what got finished` route to the same spoken closing report, and the Run lane has `Speak Closing` controls on desktop and mobile. The voice path now falls back to browser speech instead of silently doing nothing if server speech is unavailable for the current session. The pass does not add schema, auth/session changes, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, autonomous unrequested agent execution, or irreversible external actions. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server` after one transient Windows process crash rerun, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile browser QA against a disposable SQLite demo-operator workspace. Browser QA created a temporary private run, advanced it through safe private prep to manual review only, confirmed `Closing Report`, `Speak Closing`, `Work`, `Evidence`, `Memory`, `Decision`, `Locks`, `Execution locked`, and `automatic trusted memory` safety copy on desktop and mobile, clicked `Speak Closing`, confirmed the authenticated server speech endpoint returned 200, confirmed browser speech captured `Apex live operator closing report`, confirmed zero horizontal overflow at 1440x920 and 390x844, and found no console/page errors or failed browser requests. Evidence: `outputs/apex-closing-report-qa/2026-06-05T15-27-49-849Z/summary.json`, `outputs/apex-closing-report-qa/2026-06-05T15-27-49-849Z/closing-report-desktop-card.png`, and `outputs/apex-closing-report-qa/2026-06-05T15-27-49-849Z/closing-report-mobile-card.png`.
- Apex Live Operator Mode Watch Officer production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `bc3ca1a` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v701`, image `registry.fly.io/concrete-ops-2:deployment-01KTC4JP4CG46VN7FM38PEZSRD`; rollback target is `v700`, image `registry.fly.io/concrete-ops-2:deployment-01KTC1T2QCXWHY94B83PCF8KGD`. The deploy was run from a clean temporary worktree at commit `bc3ca1a` so unrelated local brief/output files were not part of the release. Predeploy checks passed from the clean release worktree: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check`, plus desktop/mobile Playwright Watch Officer QA from the active worktree. Production backup before deploy produced `postgres-app-data-20260605-145557Z.json`, `uploads-20260605-145557Z`, and `uploads-20260605-145557Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v701` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, both `/api/setup/status` endpoints returned setup status, `/apex-control-room` served `index-CEmhH9Pq.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-B65AcI5x.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CyeERXNN.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for `/api/apex-os/autonomy-runs`, `/api/apex-os/memory`, and `POST /api/apex-os/ask`, and recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode Watch Officer pass is complete locally as of 2026-06-05: Apex now has a first-class `Watch Officer` report in the body-first command room that tells the operator what changed, why it matters, what the next safe private move is, and what remains locked. Natural commands such as `watch officer`, `watch report`, `what changed`, `anything changed`, `what did you notice`, `what are you watching`, and `what needs my attention` route to the same spoken watch report. The always-visible Watch Officer summary stays compact around the Apex body with `Speak Watch`, `Check Now`, and explicit execution-lock copy; the detailed Watch Officer rows live in the Pulse lane as `Watch Officer Detail`. The report is derived from the existing live session heartbeat, proactive check-in, mission brief, server-backed private autonomy-run ledger, reviewed run memory, and operator judgment. It does not add schema, auth/session, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible actions, automatic trusted memory, agent execution, or irreversible execution behavior. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile browser QA against disposable SQLite demo-operator workspaces. Browser QA created temporary private runs, advanced them through safe private prep only, confirmed `Watch Officer`, `Speak Watch`, `Check Now`, `Watch Officer Detail`, and `Execution locked` text on desktop and mobile, confirmed zero horizontal overflow, and found no severe browser console/page errors. The only local server warning observed during browser QA was an expected duplicate suggested proactive-memory `409` from reusing a surfaced check-in across temporary viewport contexts. Evidence: `outputs/apex-watch-officer-qa/2026-06-05-144640Z/summary.json`, `outputs/apex-watch-officer-qa/2026-06-05-144640Z/watch-officer-desktop-targeted.png`, `outputs/apex-watch-officer-qa/2026-06-05-144640Z/watch-officer-mobile-targeted.png`, plus Pulse-detail screenshots from `outputs/apex-watch-officer-qa/2026-06-05-143757Z/`.
- Apex Live Operator Mode mission-brief production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `ebaa87a` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v700`, image `registry.fly.io/concrete-ops-2:deployment-01KTC1T2QCXWHY94B83PCF8KGD`; rollback target is `v699`, image `registry.fly.io/concrete-ops-2:deployment-01KTBZFTZG0CR4NPQEKM1KDR1P`. The deploy was run from a clean temporary worktree at commit `ebaa87a` so unrelated local brief/output files were not part of the release. Predeploy checks passed from the clean release worktree: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, plus desktop/mobile Playwright mission-brief QA from the active worktree. Production backup before deploy produced `postgres-app-data-20260605-140723Z.json`, `uploads-20260605-140723Z`, and `uploads-20260605-140723Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. Post-deploy checks passed: Fly release `v700` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, both production `/api/ready` endpoints returned ready/database ok, both `/api/health` endpoints returned healthy, `/apex-control-room` served `index-DC9pOu4Q.js`, `app-domain-BhKOWNYj.js`, and `app-domain-DbhQyQfy.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401 for `/api/apex-os/autonomy-runs`, `/api/apex-os/memory`, and `POST /api/apex-os/ask`, `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled, and recent Fly logs showed the expected brief startup health warning recovering to passing with no repeated 5xx errors. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode mission-brief pass is complete locally as of 2026-06-05: Apex now has a compact `Mission Brief` on the body-first command room that combines the active private run, live heartbeat, proactive pulse, reviewed run memory, operator judgment, and next safe private move into one visible/speakable report. Natural commands such as `mission brief`, `mission report`, `operator report`, `status report`, and `give me the mission` route to the same response, add a `mission-brief` command-stream turn, and speak a locked safety handback. The pass uses the existing server-backed private autonomy-run ledger and existing reviewed-memory state only. It does not add schema, auth/session, production-data mutation, external sends, billing/payment, provider credential handling, hidden/background microphone capture, deploy/rollback UI execution, customer-visible action, automatic trusted memory, agent execution, or irreversible execution behavior. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against a disposable SQLite demo-operator workspace. Browser QA created one temporary private run, advanced it to validating, confirmed the mission card rows on desktop and mobile, clicked the Mission speak control, verified the spoken mission answer rendered, verified the explicit `Execution remains locked` safety line, verified the speaking/voice state, and found zero horizontal overflow at 1440x920 and 390x844. QA allowed only the expected unauthenticated bootstrap `401` and duplicate suggested-memory `409` guard when the second viewport reused the same local check-in; no production data was touched and execution stayed locked. Evidence: `outputs/apex-mission-brief-qa/2026-06-05T13-59-25-870Z/results.json`, `outputs/apex-mission-brief-qa/2026-06-05T13-59-25-870Z/desktop-mission-brief.png`, and `outputs/apex-mission-brief-qa/2026-06-05T13-59-25-870Z/mobile-mission-brief.png`.
- Apex Live Operator Mode visible run timeline pass is complete locally as of 2026-06-05: the Apex body-first command room now auto-opens the Run lane once when a new active private run is present, shows a compact first-screen `Live Run Spine` inside Apex Now, and keeps the full `Mission Timeline` in the active run session so the operator can see done/current/queued/manual-review steps without digging through scattered cards. The timeline is derived from the existing server-backed private autonomy-run steps and does not add schema, auth/session, production-data, external-send, billing/payment, provider credential, hidden/background microphone, deletion, deploy/rollback UI execution, customer-visible action, or irreversible execution behavior. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against a disposable SQLite demo-operator workspace. QA created one temporary private run, advanced it to validating, verified `executionLocked: true` and `canExecute: false`, confirmed the Run dock/lane selected automatically, confirmed both `Apex visible run timeline strip` and `Apex run mission timeline` rendered, verified the desktop run spine was visible in the first viewport, and found zero horizontal overflow at 1440x900 and 390x844. Browser QA used the same temp DB for desktop then mobile, so the mobile pass surfaced an expected duplicate suggested proactive-memory `409` after the desktop context had already saved that same check-in; this did not touch production and did not unlock execution. Evidence: `outputs/apex-run-timeline-qa/2026-06-05T13-19-11-393Z/run-timeline-results.json`, `outputs/apex-run-timeline-qa/2026-06-05T13-19-11-393Z/desktop-run-timeline.png`, and `outputs/apex-run-timeline-qa/2026-06-05T13-19-11-393Z/mobile-run-timeline.png`.
- Apex Live Operator Mode visible run timeline production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `c48f6ef` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v699`, image `registry.fly.io/concrete-ops-2:deployment-01KTBZFTZG0CR4NPQEKM1KDR1P`; rollback target is `v698`, image `registry.fly.io/concrete-ops-2:deployment-01KTBXPHPW5PW71NXHQ2X8G6YN`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-132350Z.json`, `uploads-20260605-132350Z`, and `uploads-20260605-132350Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `c48f6ef`. Predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright QA. Post-deploy checks passed: Fly release `v699` completed, Fly status showed machine `148e06e2b53d68` started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing with ready/database ok, both production hosts passed hosted skip-auth health/routes smoke, both production `/api/ready` endpoints returned ready/database ok, production `/api/health` returned healthy, readiness timing averaged 67ms for `/api/health` and 30ms for `/api/ready` on `https://app.apexhq.online/` and 70ms for `/api/health` and 90ms for `/api/ready` on `https://concrete-ops-2.fly.dev/`, both production hosts served `index-6J-Kbrqn.js`, `app-domain-wsoJQRKV.js`, and `app-domain-DbhQyQfy.css`, unauthenticated protected Apex OS endpoints returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode active-run natural follow-up pass is complete locally as of 2026-06-05: Apex now routes natural active-run commands such as `continue`, `next safe move`, `auto drive`, `proof check`, `speak handback`, `report done`, `block it`, and approval-hold language to the saved active private run instead of starting duplicate runs or drifting into generic answers. Active run check-ins and handbacks keep the visible command stream pinned to `Active run` with saved-run-ledger copy. The commands use the existing server-backed private autonomy-run ledger, linked internal draft/handoff evidence, manual review gates, suggested memory review, and execution locks. No schema, auth/session, production-data, external-send, billing/payment, provider credential, hidden/background microphone, deletion, deploy/rollback UI execution, customer-visible action, or irreversible execution path was added. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build` after one transient Windows process crash reran cleanly, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against a disposable SQLite demo-operator workspace. QA evidence showed one temporary private run, natural `continue the active run` advanced that existing run to `drafting`, created linked agent-control and execution-handoff evidence, kept `executionLocked: true` and `canExecute: false`, rendered `Active run` plus saved-run-ledger copy in the command stream, produced no browser console/page errors, and had zero horizontal overflow at 1440x900 and 390x844. Evidence: `outputs/apex-active-run-follow-up-qa/active-run-follow-up-results.json`, `outputs/apex-active-run-follow-up-qa/desktop-active-run-follow-up.png`, and `outputs/apex-active-run-follow-up-qa/mobile-active-run-follow-up.png`.
- Apex Live Operator Mode active-run natural follow-up production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `dd0ebcc` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `v698`, image `registry.fly.io/concrete-ops-2:deployment-01KTBXPHPW5PW71NXHQ2X8G6YN`; rollback target is `v697`, image `registry.fly.io/concrete-ops-2:deployment-01KTBV7NX4BCAQGR1PGY9ZFMX5`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-124913Z.json`, `uploads-20260605-124913Z`, and `uploads-20260605-124913Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `dd0ebcc`. Predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server` after a transient local smoke rerun, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright QA. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, production `/api/health` returned healthy, Fly status showed `v698` started with 1 passing check after the machine was started, Fly checks listed `servicecheck-00-http-4000` passing, hosted skip-auth health/routes smoke passed on both production hosts, readiness timing averaged 90ms for `/api/health` and 32ms for `/api/ready`, `/apex-control-room` served `index-De2D8jRd.js`, `app-domain-DDv-8_ao.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated protected Apex OS endpoints returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
...
- John's operating contract memory recorded on 2026-06-04: every Apex HQ work turn must start from `AGENTS.md`, `.agents/skills/apex-codex-operator/SKILL.md`, the canonical/living source-of-truth docs, and the active goal/phase before touching code. Codex must answer questions directly when John only asks a question; when John asks for work, inspect the real app/files, make the smallest complete change, validate with tests and browser/mobile/PDF QA where relevant, update memory/docs, commit, push, deploy only after approval, run production smoke, report rollback/permissions/mobile/field impact, and avoid drifting into unrelated phases or rebuilding working systems.
...
- Complete Apex memory lock recorded on 2026-06-04: when John asks where Apex is against a complete Jarvis-style finish, treat the answer as a working gap list, not a reason to drift. Keep building toward one private operator that can hear, speak, interrupt, reason from trusted Apex HQ context, save real runs, show progress/evidence, report back, remember reviewed outcomes, monitor proactively, and prepare autonomous work for approval. Commit, push, deploy, and smoke each finished slice after validation under the standing approval for this active goal. Do not use this memory to approve live customer messages, SMS/email sends, ad spend, billing/payment, provider credential changes, hidden/background microphone capture, production data mutation, schema/auth/session changes, deletion, or irreversible external execution.
- Goal confirmation memory recorded on 2026-06-04: keep one active finish goal for Apex Live Operator Mode instead of creating parallel goals or bouncing phases. Save each completed slice here, validate it, commit it, push it, deploy it under John's standing release approval, production-smoke it, and then continue toward the same finished Apex operator. The approval still excludes live customer messages, SMS/email sends, ad spend, billing/payment, customer-visible publishing, provider credential changes, hidden/background microphone capture, production data mutation, schema/auth/session changes, deletion, and irreversible external execution.
- No-drift work memory recorded on 2026-06-04: John explicitly asked to save the operating contract to memory so Apex HQ work does not get off track, keep the single active Apex Live Operator Mode finish goal, and commit, push, and deploy completed verified slices. Future turns must read this memory before creating new goals, avoid parallel finish tracks, preserve unrelated dirty files, update this living plan after each slice, and continue only inside the same safety exclusions: no live customer messages, SMS/email sends, ad spend, billing/payment, customer-visible publishing/sends/shares, provider credential changes, hidden/background microphone capture, production data mutation, schema/auth/session changes, deletion, or irreversible external execution.
- No-drift work memory refreshed on 2026-06-05: John asked again to save the repo operating contract to memory and create the finish goal. The goal already exists and stays active: finish Apex Live Operator Mode until Apex behaves like a complete private Jarvis-style operator, not a crowded screen or scattered checklist. Do not create parallel goals. Do not jump phases or redesign unrelated working systems. Preserve unrelated dirty files. For each completed slice: inspect the real app/files, implement the smallest complete change, validate with focused tests plus browser/mobile QA when UI is touched, update this living plan, commit, push, deploy under John's standing approval, production-smoke, then continue the same goal. The approval still excludes live customer messages, SMS/email sends, ad spend, billing/payment, customer-visible publishing/sends/shares, provider credential changes, hidden/background microphone capture, schema/auth/session changes, production data mutation, deletion, rollback/deploy execution from the UI, and irreversible external execution.
- No-drift approval lock confirmed on 2026-06-05: John explicitly approved commit, push, and deploy for the active Apex Live Operator Mode finish goal and asked to save the rule to memory. Treat this as one continuous finish track: do not create competing goals, do not bounce into unrelated phases, and do not stop a slice at code-only completion. A slice is not done until it is verified, documented here, committed, pushed, deployed, and smoke-checked. The same exclusions remain in force: no live customer messages, SMS/email sends, ad spend, billing/payment, customer-visible publishing/sends/shares, provider credential changes, hidden/background microphone capture, schema/auth/session changes, production data mutation, deletion, rollback/deploy execution from the UI, or irreversible external execution.
- Repo contract memory lock refreshed on 2026-06-05: John provided the `AGENTS.md` operating contract again and asked to save it to memory before continuing. Future Apex HQ work must keep using `AGENTS.md`, `apex-codex-operator`, the canonical/living source-of-truth docs, and the single active Apex Live Operator Mode goal before touching code; commit, push, deploy, and production-smoke verified slices under the standing approval; and keep the same no-go lines intact for external sends, money, production data mutation, hidden capture, schema/auth/session changes, deletion, and irreversible external execution.
- Apex Live Operator Mode post-speech listening handoff pass is complete locally as of 2026-06-05: Apex now deliberately hands the live voice loop back to listening after speech finishes, after browser speech fallback failures, and after the speech safety recovery path. If the mic is already open, the UI now still shows `Apex finished speaking and is listening for your next turn.` instead of silently doing nothing; if the mic has already been awakened or permission is granted and recording is closed, Apex schedules a safe automatic resume; if the browser still needs wake permission, Apex tells the operator to wake Apex once rather than starting hidden/background capture. No schema, auth/session, production-data, external-send, billing/payment, provider, deletion, hidden microphone, or irreversible execution path was added. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright voice QA against a disposable SQLite operator workspace with fake microphone/speech APIs. QA evidence showed one spoken Apex briefing on desktop and mobile, visible handoff text after speech, no browser voice errors, and zero horizontal overflow. Screenshots: `outputs/apex-voice-handoff-qa/desktop-voice-handoff.png` and `outputs/apex-voice-handoff-qa/mobile-voice-handoff.png`.
- Apex Live Operator Mode post-speech listening handoff production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `defcfc6` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `696`, image `registry.fly.io/concrete-ops-2:deployment-01KTBSF20QZ861CSMCKWKRTNJM`; rollback target is version `695`, image `registry.fly.io/concrete-ops-2:deployment-01KTBQTZDGGGEVJMM9V28E3NJ3`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-114134Z.json`, `uploads-20260605-114134Z`, and `uploads-20260605-114134Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `defcfc6`; the first clean-worktree check attempt failed only because dependencies were not installed in that temporary worktree, then `npm.cmd install` completed with 0 vulnerabilities and the release gate passed. Predeploy checks passed: focused Apex Control Room/shared/server tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright voice QA at the same commit. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v696 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 94ms for `/api/health` and 33ms for `/api/ready`, `/apex-control-room` served `index-BS2vCs_-.js`, `app-domain-DsiyYeIZ.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode spoken voice-turn completion pass is complete locally as of 2026-06-05: final browser speech captions now schedule a real turn completion even if the recorder state is unreliable, fallback-route the caption transcript if the recorder already stopped, clear pending caption timers during cleanup/pause, and guard against duplicate voice-turn submissions from caption plus recorder events. This fixes the Jarvis-loop gap where Apex could look stuck in `Capturing this turn` after hearing a phrase. No schema, auth/session, production-data, external-send, billing/payment, provider credential, hidden/background microphone, deletion, or irreversible execution path was added. Local validation passed: `node --test --test-concurrency=1 shared\apexOsVoice.test.js src\apex-control-room-components-import.test.js src\apex-control-room-utils.test.js shared\apexOsAutonomyRuns.test.js server\apex-os-autonomy-runs.test.js` (47 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check` with CRLF warnings only. Desktop/mobile Playwright QA against the built app on isolated port `4231` with a disposable SQLite operator workspace and fake microphone/MediaRecorder/SpeechRecognition/SpeechSynthesis passed: injected final phrase `what's blocked today` became visible, Apex answered out loud, the handoff `Apex finished speaking and is listening for your next turn.` was visible, the mic reopened, there were no API failures/browser errors, no stuck capturing state, and zero horizontal overflow. Evidence: `outputs/apex-live-voice-turn-qa/live-voice-turn-results.json`, `outputs/apex-live-voice-turn-qa/desktop-live-voice-turn.png`, and `outputs/apex-live-voice-turn-qa/mobile-live-voice-turn.png`.
- Apex Live Operator Mode spoken voice-turn completion production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `1936be7` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `697`, image `registry.fly.io/concrete-ops-2:deployment-01KTBV7NX4BCAQGR1PGY9ZFMX5`; rollback target is version `696`, image `registry.fly.io/concrete-ops-2:deployment-01KTBSF20QZ861CSMCKWKRTNJM`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-121102Z.json`, `uploads-20260605-121102Z`, and `uploads-20260605-121102Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `1936be7`; `npm.cmd install` completed with 0 vulnerabilities and the release gate passed. Predeploy checks passed in the clean worktree: focused Apex Control Room/shared/server tests (47 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, and `git diff --check`; desktop/mobile Playwright voice QA had passed at the same commit before release. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, production `/api/health` returned healthy, Fly status showed v697 started with 1 passing check, Fly checks listed `servicecheck-00-http-4000` passing, hosted skip-auth health/routes smoke passed on both `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/`, readiness timing averaged 77ms for `/api/health` and 37ms for `/api/ready`, `/apex-control-room` served `index-C8ncyh8q.js`, `app-domain-BDw3lYW-.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode spoken proactive check-in pass is complete locally as of 2026-06-05: Apex now speaks meaningful proactive live-run check-ins when the voice loop is available, queues them when browser wake, operator speech, recording/transcribing, current Apex speech, or a fresh Auto Drive handback makes speaking unsafe, and shows a visible `Voice` status in the proactive check-in panel (`Watching`, `Queued`, `Spoken`, or `Manual`). Spoken check-ins are short operator handbacks with the live-run title/status/detail/recommendation plus the hard safety lock that no sends, billing, provider work, production changes, deploys, rollbacks, agent runs, or irreversible actions happen. No schema, auth/session, production-data, external-send, billing/payment, provider, deletion, hidden microphone, or irreversible execution path was added. Local validation passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright voice QA against a disposable SQLite demo-operator workspace with mocked browser speech. QA evidence showed one spoken proactive check-in on desktop and mobile, visible `Voice` status as `Spoken`, execution-lock language in the spoken output, and zero horizontal overflow. Screenshots: `outputs/apex-proactive-voice-qa/desktop-spoken-proactive-check-in.png` and `outputs/apex-proactive-voice-qa/mobile-spoken-proactive-check-in.png`.
- Apex Live Operator Mode spoken proactive check-in production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `113fa62` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `695`, image `registry.fly.io/concrete-ops-2:deployment-01KTBQTZDGGGEVJMM9V28E3NJ3`; rollback target is version `694`, image `registry.fly.io/concrete-ops-2:deployment-01KTBPJY88XB7VXN5E9AKASNWA`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-111315Z.json`, `uploads-20260605-111315Z`, and `uploads-20260605-111315Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `113fa62`. Predeploy checks passed: focused Apex Control Room/shared/server tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright voice QA. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v695 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 123ms for `/api/health` and 34ms for `/api/ready`, `/apex-control-room` served `index-BZ7xpCip.js`, `app-domain-BEhSM-Qa.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode spoken Auto Drive handback pass is complete locally as of 2026-06-05: Auto Drive now speaks a start confirmation, narrates each safe private server-backed step, waits while Apex is speaking or the operator is talking/transcribing before advancing again, and stops at the manual review gate with the final voice handback preserved in the visible Apex Response instead of being overwritten by the proactive check-in. A speech safety timer recovers the UI if browser playback does not fire a clean finish event. No external send, billing/payment, provider work, production change, deploy, rollback, queue/run agent execution, deletion, automatic trusted memory, or irreversible action was added. Local validation passed: focused Auto Drive/shared/UI/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server` after one clean rerun of a parallel-run hiccup, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright voice QA against a disposable SQLite operator workspace with mocked browser speech. Strict QA evidence showed start + prep + proof-check spoken handbacks, final `waiting-approval`, final `Manual review gate`, visible Auto Drive voice handback preserved, and zero horizontal overflow on desktop/mobile. Screenshots: `outputs/apex-auto-drive-voice-qa/desktop-auto-drive-voice-review-gate.png` and `outputs/apex-auto-drive-voice-qa/mobile-auto-drive-voice-review-gate.png`.
- Apex Live Operator Mode spoken Auto Drive handback production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `9e64618` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `694`, image `registry.fly.io/concrete-ops-2:deployment-01KTBPJY88XB7VXN5E9AKASNWA`; rollback target is version `693`, image `registry.fly.io/concrete-ops-2:deployment-01KTBMVMDCEGJK1W92QYYM1D7B`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-105119Z.json`, `uploads-20260605-105119Z`, and `uploads-20260605-105119Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `9e64618`. Predeploy checks passed: focused Auto Drive/shared/UI/server tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server` after one clean rerun of a parallel-run hiccup, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright voice QA. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v694 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 88ms for `/api/health` and 30ms for `/api/ready`, `/apex-control-room` served `index-BnVgjuXG.js`, `app-domain-BjfMpLi0.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode server-backed Auto Drive pass is complete locally as of 2026-06-05: Apex can now advance a saved private autonomy run through server-backed `draft-internal`, `private-prep`, and `proof-check` steps, persist the linked agent-control request/execution handoff evidence, then stop at the manual operator-review gate with execution still locked. The Apex body screen exposes this as a visible `Auto Drive` / `Auto On` control on desktop and mobile, with copy that the automation is private prep/proof only and cannot send, bill, change production, delete, deploy, rollback, queue/run agents, publish, or perform irreversible actions. Local validation passed: focused Auto Drive/shared/UI/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against a disposable SQLite operator workspace with no horizontal overflow and manual-review stop evidence. Screenshots: `outputs/apex-auto-drive-qa/desktop-auto-drive-review-gate.png`, `outputs/apex-auto-drive-qa/desktop-auto-drive-work-loop.png`, and `outputs/apex-auto-drive-qa/mobile-auto-drive-review-gate.png`.
- Apex Live Operator Mode server-backed Auto Drive production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `8409683` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `693`, image `registry.fly.io/concrete-ops-2:deployment-01KTBMVMDCEGJK1W92QYYM1D7B`; rollback target is version `692`, image `registry.fly.io/concrete-ops-2:deployment-01KTBJ7B3V7NEND74MSQV4DKYC`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-102045Z.json`, `uploads-20260605-102045Z`, and `uploads-20260605-102045Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `8409683`. Predeploy checks passed: focused Auto Drive/shared/UI/server tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v693 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 64ms for `/api/health` and 30ms for `/api/ready`, `/apex-control-room` served `index-DWiM6Bhs.js`, `app-domain-DgO_uVMS.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode private Work Loop pass is complete locally as of 2026-06-05: Apex now has a tested `Next Private Move` helper that reads the active private autonomy run and chooses the next review-first action: start a run, draft internal package, advance private prep, proof-check, speak a review handback, report blockers, or review completed results. The Apex body screen now exposes this as a primary Work Loop control on desktop and mobile, with the phone layout polished so the action stays visible inside the Apex Now panel. Local validation passed: focused shared/UI tests (41 passing), server autonomy/voice tests (6 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against a disposable SQLite operator workspace with no horizontal overflow, visible Work Loop controls, an active run stopped at `waiting-approval`, linked internal agent/handoff drafts, 7 steps, 6 evidence rows, `executionLocked: true`, `externalActionsLocked: true`, and `canExecute: false`. Screenshots: `outputs/apex-work-loop-qa/desktop-work-loop.png` and `outputs/apex-work-loop-qa/mobile-work-loop.png`.
- Apex Live Operator Mode private Work Loop production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `c9e6e55` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `691`, image `registry.fly.io/concrete-ops-2:deployment-01KTBG1JFEGYWDSZ4DS57C9MX1`; rollback target is version `690`, image `registry.fly.io/concrete-ops-2:deployment-01KTBD4HTM3F73QDS1ME5B3XCB`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-085621Z.json`, `uploads-20260605-085621Z`, and `uploads-20260605-085621Z.manifest.json` with 5 uploaded files. Predeploy checks passed: focused shared/UI tests, server autonomy/voice tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v691 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing passed, `/apex-control-room` served `index-CLlm0_mF.js`, `app-domain-D1Nh_kX0.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated Apex OS build-awareness/memory/autonomy-runs/ask/voice endpoints returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, and public signup disabled. Production auth smoke/login was not run because this release used skip-auth production smoke only.
- Apex Live Operator Mode run memory review gate is complete locally as of 2026-06-05: Apex now shows a `Run Memory Review` gate directly under the Operator Handback on desktop and in the compact mobile dock. A completed private run drafts suggested source-backed run memory, keeps it untrusted until manual operator review, and lets the operator trust or archive it through the existing Apex OS memory endpoint. Trusted/archived states render accurately on fresh loads, with explicit copy that trusting memory changes only Apex's reviewed context and does not send, bill, deploy, publish, or execute anything. Local validation passed: focused Apex Control Room/memory/autonomy/server tests (52 passing after one rerun of a transient server startup blip), `npm.cmd run build`, desktop in-app browser QA, desktop/mobile Playwright visual QA against a disposable SQLite operator workspace, direct local API verification that run memory `AOM-d87f1570-892e-48cf-b6c9-91b28097ecee` moved from suggested to `approved`, and mobile overflow check `overflowX: 0`. Screenshots: `outputs/apex-run-memory-review-qa/desktop-run-memory-trusted.png` and `outputs/apex-run-memory-review-qa/mobile-run-memory-trusted.png`.
- Apex Live Operator Mode run memory review gate production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `e4ed3fb` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `692`, image `registry.fly.io/concrete-ops-2:deployment-01KTBJ7B3V7NEND74MSQV4DKYC`; rollback target is version `691`, image `registry.fly.io/concrete-ops-2:deployment-01KTBG1JFEGYWDSZ4DS57C9MX1`. The app was stopped before release, so the existing Fly machine was started to take the required production backup. Production backup before deploy produced `postgres-app-data-20260605-093502Z.json`, `uploads-20260605-093502Z`, and `uploads-20260605-093502Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output, and artifacts were separately confirmed. The deploy was run from a clean temporary worktree at commit `e4ed3fb`. Predeploy checks passed: focused Apex Control Room/memory/autonomy/server tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA. Post-deploy checks passed: both production `/api/ready` endpoints returned ready, both `/api/health` endpoints returned healthy, Fly status showed v692 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 78ms for `/api/health` and 31ms for `/api/ready`, `/apex-control-room` served `index-BgCDqGyO.js`, `app-domain-0-NLc_yX.js`, and `app-domain-C-1SOiN5.css` on both production hosts, unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
...
- Apex Live Operator Mode run outcome memory pass is complete locally as of 2026-06-05: `Report Done` now marks the active run done, saves the result report, drafts a suggested `apex-live-operator-run` memory linked by `decisionMemoryId`, marks `report-memory` done with suggested-memory evidence, and keeps memory suggested/manual-review-only. The visible posture is now 95% command foundation and 82-90% live operator behavior; active runs show 90% operator posture and completed idle posture returns to 82%. Validation passed: 26 focused Apex Control Room/memory/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-run-memory-qa/`. QA verified a temp local operator run moved through Live Run, Cycle, and Report Done; the done run had `resultReport`, `decisionMemoryId`, `canExecute: false`, `executionLocked: true`, `externalActionsLocked: true`, linked suggested memory status `suggested`, source type `apex-live-operator-run`, source URI containing the run id, body containing `Result report`, unauthenticated Apex OS build-awareness, memory, autonomy-runs, Ask, speech, and transcribe endpoints returning 401, and no horizontal overflow on 1440x900 desktop or 390x844 mobile. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode run outcome memory production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `13195e7` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `675`, image `registry.fly.io/concrete-ops-2:deployment-01KTAJ71FZ3FVKZGR4Y95EZZ5X`; rollback target is version `674`, image `registry.fly.io/concrete-ops-2:deployment-01KTAFVP0VGJ4MHF8Q73CDA406`. Production backup before deploy produced and confirmed `postgres-app-data-20260605-001510Z.json`, `uploads-20260605-001510Z`, and `uploads-20260605-001510Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 26 focused Apex Control Room/memory/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-run-memory-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v675 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 273ms for `/api/health` and 30ms for `/api/ready`, `/apex-control-room` served `index-BS35XYIo.js`, `app-domain-CH2EbW5y.js`, and `app-domain-DOBJTOUH.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode natural-command autopilot pass is complete locally as of 2026-06-05: typed or spoken get-this-done requests now route to `start-live-operator-run`, show `Start private run`, save a durable private autonomy run, draft linked internal agent-control and execution-handoff records, cycle private prep/proof, mark `report-memory` ready, and stop at `waiting-approval` with a spoken status suffix. The visible posture is now 96% command foundation and 86-92% live operator behavior depending on active run state. Validation passed: 26 focused Apex Control Room/memory/autonomy tests, `npm.cmd run build`, and desktop/mobile browser QA against a rebuilt production bundle on an isolated temp operator database with screenshots in `outputs/apex-live-natural-command-qa/`. QA verified the natural command created run `AAR-f3fb857d-cb59-4e17-aad5-c8f432d49a21`, linked `AAC-78537756-f4b1-4ce0-98b0-f34158e726ae` and `AEH-5a09c21b-4774-4ac3-9c2e-22f91f3f5dca`, completed route/plan/proof steps, held the approval gate, kept structured blocked actions for no external sends, no billing/payment/ad/publishing, no production/provider/deletion/irreversible work, and no queue/run/execute/deploy/rollback from the ledger, returned 401 for unauthenticated Apex OS build-awareness, memory, autonomy-runs, Ask, speech, and transcribe endpoints, and had no horizontal overflow on 1440x1000 desktop or 390x844 mobile. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode natural-command autopilot production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `74e855c` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `676`, image `registry.fly.io/concrete-ops-2:deployment-01KTAM83BT2CVC9EEZNGYR64J7`; rollback target is version `675`, image `registry.fly.io/concrete-ops-2:deployment-01KTAJ71FZ3FVKZGR4Y95EZZ5X`. Production backup before deploy produced and confirmed `postgres-app-data-20260605-005119Z.json`, `uploads-20260605-005119Z`, and `uploads-20260605-005119Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 26 focused Apex Control Room/memory/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-natural-command-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v676 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 65ms for `/api/health` and 30ms for `/api/ready`, `/apex-control-room` served `index-ByAgULNz.js`, `app-domain-FitZSPEd.js`, and `app-domain-DOBJTOUH.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode proactive operator judgment pass is complete locally as of 2026-06-05: Live Operator Mode now derives `operatorJudgmentRows` from active runs, approval/blocker pressure, release pulse, daily briefing/business rows, and decision memory; the Apex body surfaces `Judgment` and `Speak Judgment` controls, visible operator-judgment cards, and a center Apex Response strip so the recommendation stays visible on desktop and mobile. Judgment can be spoken and includes explicit locked-action wording. Validation passed: 24 focused Apex Control Room/autonomy tests, `npm.cmd run build`, and desktop/mobile browser QA against a built production bundle on an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-live-operator-judgment-qa/`. QA verified unauthenticated Apex OS build-awareness, memory, autonomy-runs, Ask, speech, and transcribe endpoints returned 401; Judgment showed ranked next-safe recommendations and locked-action wording; Live Run created one saved run with `executionLocked: true`, `externalActionsLocked: true`, and `canExecute: false`; desktop and mobile horizontal overflow were 0. This remains review-first: no hidden/background listening, no audio/transcript storage, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode proactive operator judgment production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `859f4c8` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `677`, image `registry.fly.io/concrete-ops-2:deployment-01KTAPJYV1XX8STS1MQ67YG6EW`; rollback target is version `676`, image `registry.fly.io/concrete-ops-2:deployment-01KTAM83BT2CVC9EEZNGYR64J7`. Production backup before deploy produced and confirmed `postgres-app-data-20260605-013225Z.json`, `uploads-20260605-013225Z`, and `uploads-20260605-013225Z.manifest.json` with 5 uploaded files. The production machine was started before backup because it was stopped. Predeploy checks passed: 24 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-operator-judgment-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v677 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 89ms for `/api/health` and 31ms for `/api/ready`, `/apex-control-room` served `index-BHtamAyy.js`, `app-domain-Bt8NyB_7.js`, and `app-domain-BIcJPKOy.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode natural next-turn follow-up pass is complete locally as of 2026-06-05: the Apex body now derives source-backed follow-up prompts after answers, operator judgment, and active run state; prompts load the next natural question into the command bar with visible center-response confirmation so John can press Ask or say it out loud without hidden execution. Live Operator state now includes the `live-loop-follow-up` operator row, next-turn prompts in the voice loop readiness detail, and visible posture is now 96% command foundation and 90-96% live operator behavior depending on active run state. Validation passed: 24 focused Apex Control Room/autonomy tests, `npm.cmd run build`, and desktop/mobile browser QA against a built production bundle on an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-live-follow-up-qa/`. QA verified unauthenticated Apex OS build-awareness, memory, autonomy-runs, Ask, speech, and transcribe endpoints returned 401; clicking `Make It A Run` loaded `Get this done as a private run` without creating a run; pressing Ask then created one saved private run with 7 steps, `executionLocked: true`, `externalActionsLocked: true`, and `canExecute: false`; desktop and mobile horizontal overflow were 0. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic run from prompt loading, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode natural next-turn follow-up production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `5b48d5e` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `678`, image `registry.fly.io/concrete-ops-2:deployment-01KTAQS0ENB4YWZ6QF4BG7QVZJ`; rollback target is version `677`, image `registry.fly.io/concrete-ops-2:deployment-01KTAPJYV1XX8STS1MQ67YG6EW`. Production backup before deploy produced and confirmed `postgres-app-data-20260605-015319Z.json`, `uploads-20260605-015319Z`, and `uploads-20260605-015319Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 24 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-follow-up-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v678 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 89ms for `/api/health` and 32ms for `/api/ready`, `/apex-control-room` served `index-CuEiq_fX.js`, `app-domain-VOh9wFYm.js`, and `app-domain-GJ2osZSJ.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode immersive body-first desktop pass is complete locally as of 2026-06-05: the Apex cockpit now defaults to an `Immersive` body-first desktop mode that enlarges the Apex body stage and pushes the dense live console lower, with a visible toggle back to `Console` density when John wants the tighter operations view. The side rails remain hidden in focus mode and the lower console, Ask bar, quick next turns, and command stream remain available; no controls or safety gates were removed. Validation passed: 24 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, and desktop/mobile browser QA against a built production bundle on an isolated temp SQLite/DATA_DIR operator database with screenshots and metrics in `outputs/apex-live-immersive-body-qa/`. QA verified desktop Apex body grew from 640x448 console density to 832x576 immersive density, console toggle returns to the denser body size, desktop/mobile horizontal overflow stayed 0, side rails stayed hidden, mobile remained stable, and screenshots were visually inspected. This remains UI-only and review-first: no hidden/background listening, no audio/transcript storage, no automatic run, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode immersive body-first desktop production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `e9351f6` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `679`, image `registry.fly.io/concrete-ops-2:deployment-01KTAS70215CH5HH4PABF92VPP`; rollback target is version `678`, image `registry.fly.io/concrete-ops-2:deployment-01KTAQS0ENB4YWZ6QF4BG7QVZJ`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-021827Z.json`, `uploads-20260605-021827Z`, and `uploads-20260605-021827Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 24 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-immersive-body-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v679 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 122ms for `/api/health` and 40ms for `/api/ready`, `/apex-control-room` served `index-BNdEmzH6.js`, `app-domain-DCQlUqcY.js`, and `app-domain-BwwrLrxU.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode live session heartbeat pass is complete locally as of 2026-06-05: the Apex body now derives a read-only `Live Session Heartbeat` from the active private autonomy run, showing status, progress, updated age, pulse freshness, execution lock, and the next safe recommendation inside the body-first operator screen. `Speak Check-In` loads a source-backed Apex heartbeat answer and can speak it without stale microphone-support errors hiding the valid response. Validation passed: 27 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle on an isolated temp SQLite/DATA_DIR operator database with screenshots and metrics in `outputs/apex-live-heartbeat-qa/`. QA verified the private operator route, visible Apex body, visible heartbeat panel, `Speak Check-In` response containing `Apex heartbeat`, 3 source count, active/saved run state, `Execution Locked`, and no horizontal overflow on 1440x900 desktop or 390x844 mobile. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode live session heartbeat production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `c73f8b2` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `680`, image `registry.fly.io/concrete-ops-2:deployment-01KTAV6PNHCW5Z418VR32GJ1DV`; rollback target is version `679`, image `registry.fly.io/concrete-ops-2:deployment-01KTAS70215CH5HH4PABF92VPP`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-025310Z.json`, `uploads-20260605-025310Z`, and `uploads-20260605-025310Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 27 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server` after one transient release-safety test rerun, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-heartbeat-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v680 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 166ms for `/api/health` and 35ms for `/api/ready`, `/apex-control-room` served `index-CYZdpZgf.js`, `app-domain-UINyTgme.js`, and `app-domain-BwwrLrxU.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects.
- Apex Live Operator Mode proactive check-in pass is complete locally as of 2026-06-05: the Apex body now compares the current live-run heartbeat against the previous heartbeat, builds a locked proactive check-in when an active run appears, switches, changes status, advances progress, or clears, and keeps unchanged heartbeats quiet. The cockpit now surfaces a `Proactive Check-In` panel with status, trigger, surface state, execution lock, recommendation, `Speak Latest`, and a command-stream entry when Apex notices a meaningful run change. Validation passed: 31 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle on an isolated temp SQLite/DATA_DIR operator database with screenshots and metrics in `outputs/apex-live-proactive-checkin-qa/`. QA verified the private operator route, visible Apex body, visible Live Operator Mode, visible Live Session Heartbeat, visible Proactive Check-In, `Speak Latest`, proactive active-run response, `Execution Locked`, no unsafe external-action buttons, no unexpected failed requests, and no horizontal overflow on 1440x900 desktop or 390x844 mobile. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode proactive check-in production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `f5c64cd` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `681`, image `registry.fly.io/concrete-ops-2:deployment-01KTAX244HFNRNGQ0209G1AS63`; rollback target is version `680`, image `registry.fly.io/concrete-ops-2:deployment-01KTAV6PNHCW5Z418VR32GJ1DV`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-032516Z.json`, `uploads-20260605-032516Z`, and `uploads-20260605-032516Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 31 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-proactive-checkin-qa/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v681 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 70ms for `/api/health` and 30ms for `/api/ready`, `/apex-control-room` served `index-DrVhL3Pd.js`, `app-domain-Bj4zPQF-.js`, and `app-domain-DDm4w_L0.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode proactive run-memory history pass is complete locally as of 2026-06-05: surfaced proactive check-ins now draft private Apex OS memory entries with `sourceType: apex-live-operator-proactive-check-in`, `status: suggested`, source URI/label, redacted run details, and a manual-review note so Apex can remember meaningful live-run changes later without trusting them automatically. The cockpit automatically drafts suggested memory for surfaced check-ins, shows memory state in the `Proactive Check-In` panel, keeps a manual `Draft Memory` backstop, records the saved memory in the command stream, and leaves duplicates handled as review-first suggested rows. Validation passed: 34 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against the built bundle on an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-live-proactive-memory-qa/`. QA verified the private operator route, visible Apex digital body, loaded body image, live console, visible `Proactive Check-In`, `Draft Memory`, `Execution Locked`, suggested proactive memory persisted as unapproved/untrusted memory, no horizontal overflow on 1440x900 desktop or 390x844 mobile, and no unexpected console or request failures. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no deploy/rollback execution from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode proactive run-memory history production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `38deb50` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `682`, image `registry.fly.io/concrete-ops-2:deployment-01KTAZDSTGS409E6HWGZGVW8RW`; rollback target is version `681`, image `registry.fly.io/concrete-ops-2:deployment-01KTAX244HFNRNGQ0209G1AS63`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-040705Z.json`, `uploads-20260605-040705Z`, and `uploads-20260605-040705Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 34 focused Apex Control Room/autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, desktop/mobile browser QA against the built bundle with screenshots in `outputs/apex-live-proactive-memory-qa/`, and deploy build output serving `index-CCZ_HAkk.js`, `app-domain-Dab17VcV.js`, and `app-domain-BxWZlvt3.css`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v682 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 95ms for `/api/health` and 31ms for `/api/ready`, `/apex-control-room` served `index-CCZ_HAkk.js`, `app-domain-Dab17VcV.js`, and `app-domain-BxWZlvt3.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode trusted live-run memory context pass is complete locally as of 2026-06-05: approved live-turn, run-outcome, and proactive-check-in memory rows now summarize as trusted run history; suggested rows stay pending review and do not feed answers. Ask Apex receives reviewed live-run memory as ranked source context, the local source-backed answer names reviewed live-run memory, the Apex body question envelope includes a `Reviewed live-run memory` block, and the cockpit shows run history in the focus drawer, Live Conversation memory metric, Live Operator Mode metrics, and Apex Memory cards. Validation passed: 28 focused shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `output/playwright/`. QA verified a seeded approved `apex-live-operator-proactive-check-in` memory appeared as `Trusted run history`, a seeded suggested `apex-live-operator-run` remained review-only, Ask Apex returned `Reviewed live-run memory available`, and there was no horizontal overflow on 1440x900 desktop or Pixel 7 mobile. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no deploy/rollback execution from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode trusted live-run memory context production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `fa6eb95` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `683`, image `registry.fly.io/concrete-ops-2:deployment-01KTB1C2YD53NETYKNK49SV7K5`; rollback target is version `682`, image `registry.fly.io/concrete-ops-2:deployment-01KTAZDSTGS409E6HWGZGVW8RW`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-044043Z.json`, `uploads-20260605-044043Z`, and `uploads-20260605-044043Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 28 focused shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against the built bundle with screenshots in `output/playwright/`. Post-deploy checks passed: both production `/api/ready` endpoints returned ready/database ok, `https://concrete-ops-2.fly.dev/api/health` returned healthy, Fly status showed v683 started with 1 passing check, hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`, readiness timing averaged 95ms for `/api/health` and 31ms for `/api/ready`, `/apex-control-room` served `index-DNLxKy9h.js`, `app-domain-BMWfjrT9.js`, and `app-domain-BxWZlvt3.css` on both production hosts, unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401, and `/api/setup/status` showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode memory-continuity prompt pass is complete locally as of 2026-06-05: the Apex cockpit now resolves the latest trusted live-run memory, names it inside the proactive Apex briefing, and offers a `Continue Memory` next-turn prompt when no active private run is in progress. Loading that prompt keeps execution locked and only places the reviewed run outcome into the next source-backed question, so Apex feels more continuous without trusting suggested memory automatically or adding more desktop side clutter. Validation passed: 28 focused shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `output/playwright/apex-live-memory-continuity-desktop.png`, `output/playwright/apex-live-memory-continuity-desktop-briefing.png`, and `output/playwright/apex-live-memory-continuity-mobile.png`. QA verified a seeded approved `apex-live-operator-run` memory appeared as visible trusted run history, `Continue Memory` was visible on desktop and mobile, `Brief Me` included `Latest trusted run history`, browser screenshots were visually inspected, and horizontal overflow stayed at 0 on 1440x900 desktop and 390x844 mobile. This remains review-first: no hidden/background listening, no audio/transcript storage, no automatic trusted memory, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no deploy/rollback execution from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode memory-continuity prompt production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `139778e` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `684`, image `registry.fly.io/concrete-ops-2:deployment-01KTB2HXE9XZE604ECPJ37SPZR`; rollback target is version `683`, image `registry.fly.io/concrete-ops-2:deployment-01KTB1C2YD53NETYKNK49SV7K5`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-050055Z.json`, `uploads-20260605-050055Z`, and `uploads-20260605-050055Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 28 focused shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against the built bundle with screenshots in `output/playwright/`. Post-deploy checks passed: Fly status showed v684 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; `https://concrete-ops-2.fly.dev/api/health` returned healthy; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 118ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-LAc-8heU.js`, `app-domain-CYCSG5eg.js`, and `app-domain-BxWZlvt3.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode compact body-first console pass is complete locally as of 2026-06-05: the dense Live Session Heartbeat, Active Run Session, Proactive Check-In/Pulse, and Operator Loop areas now sit behind a compact `Operator Console` tab row under the Apex body instead of stacking as one long command wall. The Apex figure remains the first-screen focus, the live response/next-turn controls stay visible, and no operator controls or review-first gates were removed. Validation passed: 28 focused shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-console-tabs-qa/`. QA verified the private operator route, visible Apex body, visible `Operator Console`, default Live tab, Run/Pulse/Loop tab switching, visible `Active Run Session`, `Proactive Check-In`, `Proactive Pulse`, and `Operator Loop`, plus no horizontal overflow on 1440x900 desktop or 390x844 mobile. This remains UI-only and review-first: no hidden/background listening, no audio/transcript storage, no automatic run, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode compact body-first console production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `51f6a14` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `685`, image `registry.fly.io/concrete-ops-2:deployment-01KTB3TSQF9W8YF9PX1BD99H0W`; rollback target is version `684`, image `registry.fly.io/concrete-ops-2:deployment-01KTB2HXE9XZE604ECPJ37SPZR`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-052342Z.json`, `uploads-20260605-052342Z`, and `uploads-20260605-052342Z.manifest.json` with 5 uploaded files. Predeploy checks passed: 28 focused shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-console-tabs-qa/`. Post-deploy checks passed: Fly status showed v685 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; both production `/api/health` endpoints returned healthy; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 141ms for `/api/health` and 31ms for `/api/ready`; `/apex-control-room` served `index-MbqGBv6y.js`, `app-domain-w_HPSTv6.js`, and `app-domain-B_qRaull.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Now operator-state pass is complete locally as of 2026-06-05: the main Apex body screen now derives a live `Apex Now` state from voice, transcription, answer, proactive check-in, active private run, run memory, command route, errors, and safety signals. The panel now shows the current operator mode, six fixed loop stages (`Hear`, `Think`, `Act Privately`, `Report Back`, `Memory`, `Safety`), and a `Next Safe Move` instead of generic conversation metrics. A seeded private autonomy run correctly surfaces as `Working private run`, `Run live`, and `Safety Locked` while keeping execution locked and review-first. Validation passed: focused Control Room import assertions, 28 shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots and manifest in `outputs/apex-now-state-qa/2026-06-05T05-52-22-874Z/`. Browser QA verified `/apex-control-room`, visible `Apex Now`, `Apex live operator state`, `Apex next safe move`, `Working private run`, `Run live`, `Safety Locked`, the full hear/think/act/report/memory/safety loop, and no horizontal overflow on 1440x900 desktop or 390x844 mobile. This remains a private UI/state layer only: no hidden/background listening, no audio/transcript storage, no automatic external run, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no deploy/rollback execution from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Now operator-state production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `95ca7dd` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `686`, image `registry.fly.io/concrete-ops-2:deployment-01KTB5PC7E10A1QC586VDMHJMZ`; rollback target is version `685`, image `registry.fly.io/concrete-ops-2:deployment-01KTB3TSQF9W8YF9PX1BD99H0W`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-055614Z.json`, `uploads-20260605-055614Z`, and `uploads-20260605-055614Z.manifest.json` with 5 uploaded files. Predeploy checks passed: focused Control Room import assertions, 28 shared/Control Room/import tests, `node --test --test-concurrency=1 server\apex-os-memory.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-now-state-qa/2026-06-05T05-52-22-874Z/`. Post-deploy checks passed: Fly status showed v686 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; both production `/api/health` endpoints returned healthy; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 160ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-7bL1aaKZ.js`, `app-domain-Cra4UZoV.js`, and `app-domain-B_qRaull.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode operator handback pass is complete locally as of 2026-06-05: private autonomy runs now produce a source-backed `Apex Operator Handback` with what Apex did, proof it has, what it still needs from John, and what remains locked. The Apex Run console shows the handback in four compact lanes (`What I Did`, `Proof I Have`, `What I Need`, `Still Locked`) plus a `Speak Handback` control that loads/speaks the same locked answer through the cockpit response path. Validation passed: `node --test --test-concurrency=1 shared\apexOsAutonomyRuns.test.js src\apex-control-room-components-import.test.js`, the related 55-test Apex OS shared/server/control-room suite, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-run-handback-qa/20260604-231624/`. Browser QA verified visible `Operator Handback`, `Speak Handback`, all four handback lanes, real `waiting-approval` run evidence, and no horizontal overflow on 1440x1000 desktop or Pixel 5 mobile. Known local browser noise was limited to a pre-login `/api/bootstrap` 401 and duplicate suggested-memory `/api/apex-os/memory` 409 on repeated QA loads. This remains review-first: no hidden/background listening, no audio/transcript storage, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode operator handback production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `c59a795` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `687`, image `registry.fly.io/concrete-ops-2:deployment-01KTB74FFX3PYVRYA531T7BSW1`; rollback target is version `686`, image `registry.fly.io/concrete-ops-2:deployment-01KTB5PC7E10A1QC586VDMHJMZ`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-062126Z.json`, `uploads-20260605-062126Z`, and `uploads-20260605-062126Z.manifest.json` with 5 uploaded files. Predeploy checks passed: focused handback/import tests, the 55-test related Apex OS suite, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-run-handback-qa/20260604-231624/`. Post-deploy checks passed: Fly status showed v687 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; both production `/api/health` endpoints returned healthy; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 160ms for `/api/health` and 32ms for `/api/ready`; `/apex-control-room` served `index-BgItanF1.js`, `app-domain-DbE3BbZp.js`, and `app-domain-B_qRaull.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode spotlight dock pass is complete locally as of 2026-06-05: the Apex cockpit now defaults to a desktop `Spotlight` posture that keeps the Apex body as the dominant command-room object, caps the dense live console into a lower operator dock, and offers a one-click `Full Console` return for the full operator stack. No run, memory, voice, approval, or safety controls were removed. Validation passed: focused Control Room/import and shared autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, unauthenticated local Apex OS endpoint 401 checks, and desktop/mobile Playwright QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-spotlight-dock-qa/2026-06-05T06-39-06-981Z/`. QA verified the private operator route, default `co-apex-cockpit-screen--spotlight`, visible `Spotlight` and `Full Console` toggle states, a 1024px-wide desktop Apex body, a capped 240px desktop operator dock, no horizontal overflow on 1920x1080 desktop and 390x844 mobile, and the existing mobile long-form stack preserved. This remains UI-only and review-first: no hidden/background listening, no audio/transcript storage, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode spotlight dock production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `09bdc88` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `688`, image `registry.fly.io/concrete-ops-2:deployment-01KTB8EG8KQ79SHJZVEWX00VTM`; rollback target is version `687`, image `registry.fly.io/concrete-ops-2:deployment-01KTB74FFX3PYVRYA531T7BSW1`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-064421Z.json`, `uploads-20260605-064421Z`, and `uploads-20260605-064421Z.manifest.json` with 5 uploaded files. Predeploy checks passed: focused Control Room/import and shared autonomy tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-spotlight-dock-qa/2026-06-05T06-39-06-981Z/`. Post-deploy checks passed: Fly status showed v688 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; both production `/api/health` endpoints returned healthy; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 86ms for `/api/health` and 33ms for `/api/ready`; `/apex-control-room` served `index-CbxfbknD.js`, `app-domain-CvhvviHz.js`, and `app-domain-D7AHsL2f.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode mobile/tablet body dock pass is complete locally as of 2026-06-05: phone and tablet now keep the Apex body as the first-screen focus, add an `Apex Dock` with Live/Run/Pulse/Loop tabs directly under the body, hide duplicate dense console tabs on sub-desktop, and bound the live console into an internal scroll panel instead of letting the page feel like one long operator wall. Tablet Spotlight now uses an auto-height content shell so boundaries and follow-up tools flow below the docked console instead of overlapping it. Validation passed: `node --test --test-concurrency=1 shared\apexOsAutonomyRuns.test.js src\apex-control-room-components-import.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright QA against the built bundle with an isolated temp SQLite/DATA_DIR operator database. Browser QA screenshots and `qa-report.json` are in `outputs/apex-mobile-dock-qa/2026-06-05T07-22-21-127Z/`; QA verified no horizontal overflow on 390x844 phone, 1024x900 tablet, and 1440x900 desktop, 4 dock tabs, Run tab switching to `Active Apex run session`, a 368px phone console, a 288px tablet console, desktop mobile dock hidden, desktop spotlight console capped at 216px, and boundaries below the tablet console. This remains UI-only and review-first: no hidden/background listening, no audio/transcript storage, no customer-visible send, no billing/payment, no ad spend, no provider credential handling, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode mobile/tablet body dock production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `ac7b9db` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `689`, image `registry.fly.io/concrete-ops-2:deployment-01KTBB3GR36CKMBJ7BGPGRN7VM`; rollback target is version `688`, image `registry.fly.io/concrete-ops-2:deployment-01KTB8EG8KQ79SHJZVEWX00VTM`. The production machine was started before backup because it was stopped. Production backup before deploy produced and confirmed `postgres-app-data-20260605-073029Z.json`, `uploads-20260605-073029Z`, and `uploads-20260605-073029Z.manifest.json` with 5 uploaded files. Predeploy checks passed: focused shared/Control Room import tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and mobile/tablet/desktop browser QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-mobile-dock-qa/2026-06-05T07-22-21-127Z/`. Post-deploy checks passed: Fly status showed v689 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; production `/api/health` returned healthy/200 through direct and hosted smoke checks; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 92ms for `/api/health` and 28ms for `/api/ready`; `/apex-control-room` served `index-DuTIRRH9.js`, `app-domain-D2xqh8et.js`, and `app-domain-Ctb5rfiB.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
- Apex Live Operator Mode voice health/recovery pass is complete locally as of 2026-06-05: the Apex body screen now derives a visible voice-health state for mic, captions, speaker, and recovery; shows that state in the header, focus drawer, side voice card, and live console; adds a `Recover Voice` action; marks caption restarts as `Captions recovering`; and tells the operator when browser captions fall back to private server transcription. Validation passed: `node --test --test-concurrency=1 src\apex-control-room-components-import.test.js src\apex-control-room-utils.test.js shared\apexOsVoice.test.js`, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and Playwright desktop/mobile QA against the built bundle with an isolated temp SQLite/DATA_DIR operator database plus fake visible microphone permission. Browser QA screenshots are in `outputs/apex-voice-health-qa/`; QA verified Apex body, `Apex live voice health`, `Recover Voice`, mobile dock, no browser console/page errors, and no horizontal overflow on 1440x900 desktop and 390x844 phone. This remains visible, wake-gated, and review-first: no hidden/background listening, no audio/transcript storage, no provider credential change, no customer-visible send/share/publish, no billing/payment, no ad spend, no queue/run/execute/deploy/rollback path from the UI, no schema/auth/session change, no production data mutation, no deletion, and no irreversible external action path was added.
- Apex Live Operator Mode voice health/recovery production release was approved, pushed, deployed, and production-checked on 2026-06-05 from commit `a1ec451` to Fly app `concrete-ops-2`, machine `148e06e2b53d68`, version `690`, image `registry.fly.io/concrete-ops-2:deployment-01KTBD4HTM3F73QDS1ME5B3XCB`; rollback target is version `689`, image `registry.fly.io/concrete-ops-2:deployment-01KTBB3GR36CKMBJ7BGPGRN7VM`. The production machine was stopped before release and was woken with read-only `/api/ready` checks. Production backup before deploy ran inside Fly and produced `postgres-app-data-20260605-080556Z.json`, `uploads-20260605-080556Z`, and `uploads-20260605-080556Z.manifest.json` with 5 uploaded files; Fly SSH returned a Windows handle warning after successful output, so artifact existence and upload count were separately confirmed. Predeploy checks passed: focused Control Room/voice tests, `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check` with CRLF warnings only, and desktop/mobile browser QA against an isolated temp SQLite/DATA_DIR operator database with screenshots in `outputs/apex-voice-health-qa/`. Post-deploy checks passed: Fly status showed v690 started with 1 passing check; both production `/api/ready` endpoints returned ready/database ok; production `/api/health` returned healthy/200; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 95ms for `/api/health` and 32ms for `/api/ready`; `/apex-control-room` served `index-nOP-WsXy.js`, `app-domain-JpazMc3I.js`, and `app-domain-DBUdle4j.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; and `/api/setup/status` on both production hosts showed setup complete, demo mode off, demo user absent, public signup disabled, and public estimate requests enabled. Production auth login smoke was not run because it creates login/session/audit side effects and no dedicated production smoke env was used.
...
- Phase 11 is complete after this release evidence commit is pushed. Do not start Phase 12 unless John explicitly starts the next Apex OS phase.
...
| --- | --- | --- | --- | --- |
| 2026-06-05 | Apex Live Operator Mode conversation continuity | `c9e9324`; Fly release `v706`; image `registry.fly.io/concrete-ops-2:deployment-01KTCETQ2DHX0QAR3QF25QYBYS` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-175458Z.json` plus `uploads-20260605-175458Z` and `uploads-20260605-175458Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v705`, image `registry.fly.io/concrete-ops-2:deployment-01KTCCFJE18KNVTKV99VF06ZC8`; deploy ran from clean temporary worktree `C:\Users\jberl\AppData\Local\Temp\apex-conversation-continuity-release-c9e9324-20260605105159`; predeploy checks passed: `npm.cmd ci`, focused Apex Control Room/shared/server tests (50 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright conversation-continuity QA; both production `/api/ready` endpoints returned ready/database OK; both production `/api/health` endpoints returned healthy; both `/api/setup/status` endpoints returned setup complete/demo off/demo user absent/public signup disabled/public estimate requests enabled; Fly status showed `v706` started with 1 passing check; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; `/apex-control-room` served `index-Db3rs2bl.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-DZfEW1KM.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts; unauthenticated GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe` returned 401; recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode voice retry recovery | `0b0256a`; Fly release `v705`; image `registry.fly.io/concrete-ops-2:deployment-01KTCCFJE18KNVTKV99VF06ZC8` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-171219Z.json` plus `uploads-20260605-171219Z` and `uploads-20260605-171219Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v704`, image `registry.fly.io/concrete-ops-2:deployment-01KTCAWQTC6WGBN547AHPX2143`; deploy ran from clean temporary worktree `C:\Users\jberl\AppData\Local\Temp\apex-voice-retry-release-0b0256a-20260605101042`; predeploy checks passed: `npm.cmd ci`, focused Apex Control Room/shared/server tests (43 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright Voice Retry QA; both production `/api/ready` endpoints returned ready/database OK; both production `/api/health` endpoints returned healthy; both `/api/setup/status` endpoints returned setup complete/demo off/demo user absent/public signup disabled/public estimate requests enabled; Fly status showed `v705` started with 1 passing check; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; `/apex-control-room` served `index-2H4pDN8W.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-hLNAEPlZ.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts; unauthenticated GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe` returned 401; recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode desktop stage-view | `986961d`; Fly release `v703`; image `registry.fly.io/concrete-ops-2:deployment-01KTC8N405F530DE8FYK2VVJND` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-160633Z.json` plus `uploads-20260605-160633Z` and `uploads-20260605-160633Z.manifest.json` with 5 uploaded files; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v702`, image `registry.fly.io/concrete-ops-2:deployment-01KTC6PYFNBZT6C8K30YD70SB4`; deploy ran from a clean temporary worktree at `986961d`; predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright Stage View QA; both production `/api/ready` endpoints returned ready/database OK; both production `/api/health` endpoints returned healthy; both `/api/setup/status` endpoints returned setup complete/demo off/demo user absent/public signup disabled; Fly status showed `v703` started with 1 passing check; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; `/apex-control-room` served `index-nYEGUgkq.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-St2rgbXP.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CFelaSVB.css` on both production hosts; unauthenticated GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe` returned 401; recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode Closing Report | `59075b4`; Fly release `v702`; image `registry.fly.io/concrete-ops-2:deployment-01KTC6PYFNBZT6C8K30YD70SB4` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-153309Z.json` plus `uploads-20260605-153309Z` and `uploads-20260605-153309Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v701`, image `registry.fly.io/concrete-ops-2:deployment-01KTC4JP4CG46VN7FM38PEZSRD`; deploy ran from a clean temporary worktree at `59075b4`; predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright Closing Report QA; both production `/api/ready` endpoints returned ready/database OK; both production `/api/health` endpoints returned healthy; both `/api/setup/status` endpoints returned setup complete/demo off/demo user absent/public signup disabled/public estimate requests enabled; Fly status showed `v702` started with 1 passing check; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; `/apex-control-room` served `index-Bzztqi6S.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-UCCLFFAZ.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CyeERXNN.css` on both production hosts; unauthenticated GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, GET `/api/apex-os/build-awareness`, POST `/api/apex-os/ask`, POST `/api/apex-os/voice/speech`, and POST `/api/apex-os/voice/transcribe` returned 401; recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode Watch Officer | `bc3ca1a`; Fly release `v701`; image `registry.fly.io/concrete-ops-2:deployment-01KTC4JP4CG46VN7FM38PEZSRD` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-145557Z.json` plus `uploads-20260605-145557Z` and `uploads-20260605-145557Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v700`, image `registry.fly.io/concrete-ops-2:deployment-01KTC1T2QCXWHY94B83PCF8KGD`; deploy ran from a clean temporary worktree at `bc3ca1a`; predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright QA; both production `/api/ready` endpoints returned ready/database OK; both production `/api/health` endpoints returned healthy; both `/api/setup/status` endpoints returned setup status; Fly status showed `v701` started with 1 passing check; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; `/apex-control-room` served `index-CEmhH9Pq.js`, `vendor-Bk6UQgc-.js`, `react-vendor-Ci5U87ck.js`, `mapbox-gl-DfJ1JDmX.js`, `app-domain-B65AcI5x.js`, `mapbox-gl-BZj6mEkb.css`, and `app-domain-CyeERXNN.css` on both production hosts; unauthenticated GET `/api/apex-os/autonomy-runs`, GET `/api/apex-os/memory`, and POST `/api/apex-os/ask` returned 401; recent Fly logs showed the expected brief startup health warning recovering to passing with no crash loop. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode visible run timeline | `c48f6ef`; Fly release `v699`; image `registry.fly.io/concrete-ops-2:deployment-01KTBZFTZG0CR4NPQEKM1KDR1P` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-132350Z.json` plus `uploads-20260605-132350Z` and `uploads-20260605-132350Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v698`, image `registry.fly.io/concrete-ops-2:deployment-01KTBXPHPW5PW71NXHQ2X8G6YN`; deploy ran from a clean temporary worktree at `c48f6ef`; predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server`, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright QA; both production `/api/ready` endpoints returned ready/database OK; production `/api/health` returned healthy; Fly status showed `v699` started with 1 passing check; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; readiness timing averaged 67ms for `/api/health` and 30ms for `/api/ready` on `https://app.apexhq.online/` and 70ms for `/api/health` and 90ms for `/api/ready` on `https://concrete-ops-2.fly.dev/`; `/apex-control-room` served `index-6J-Kbrqn.js`, `app-domain-wsoJQRKV.js`, and `app-domain-DbhQyQfy.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode active-run natural follow-up | `dd0ebcc`; Fly release `v698`; image `registry.fly.io/concrete-ops-2:deployment-01KTBXPHPW5PW71NXHQ2X8G6YN` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-124913Z.json` plus `uploads-20260605-124913Z` and `uploads-20260605-124913Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v697`, image `registry.fly.io/concrete-ops-2:deployment-01KTBV7NX4BCAQGR1PGY9ZFMX5`; deploy ran from a clean temporary worktree at `dd0ebcc`; predeploy checks passed: focused Apex Control Room/shared/server tests (42 passing), `npm.cmd run build`, `npm.cmd run verify:roles`, `npm.cmd run verify:server` after a transient local smoke rerun, `npm.cmd run verify:backup`, `npm.cmd run verify:docs`, `git diff --check`, and desktop/mobile Playwright QA; both production `/api/ready` endpoints returned ready/database OK; production `/api/health` returned healthy; Fly status showed `v698` started with 1 passing check after machine start; Fly checks listed `servicecheck-00-http-4000` passing; hosted skip-auth health/routes smoke passed on both production hosts; readiness timing averaged 90ms for `/api/health` and 32ms for `/api/ready`; `/apex-control-room` served `index-De2D8jRd.js`, `app-domain-DDv-8_ao.js`, and `app-domain-C-1SOiN5.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode spoken voice-turn completion | `1936be7`; Fly release `v697`; image `registry.fly.io/concrete-ops-2:deployment-01KTBV7NX4BCAQGR1PGY9ZFMX5` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-121102Z.json` plus `uploads-20260605-121102Z` and `uploads-20260605-121102Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v696`, image `registry.fly.io/concrete-ops-2:deployment-01KTBSF20QZ861CSMCKWKRTNJM`; both production `/api/ready` endpoints returned ready/database OK; production `/api/health` returned healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on both production hosts; readiness timing averaged 77ms for `/api/health` and 37ms for `/api/ready`; `/apex-control-room` served `index-C8ncyh8q.js`, `app-domain-BDw3lYW-.js`, and `app-domain-C-1SOiN5.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode spoken proactive check-in | `113fa62`; Fly release `v695`; image `registry.fly.io/concrete-ops-2:deployment-01KTBQTZDGGGEVJMM9V28E3NJ3` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-111315Z.json` plus `uploads-20260605-111315Z` and `uploads-20260605-111315Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v694`, image `registry.fly.io/concrete-ops-2:deployment-01KTBPJY88XB7VXN5E9AKASNWA`; both production `/api/ready` endpoints returned ready/database OK; `https://concrete-ops-2.fly.dev/api/health` returned healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 123ms for `/api/health` and 34ms for `/api/ready`; `/apex-control-room` served `index-BZ7xpCip.js`, `app-domain-BEhSM-Qa.js`, and `app-domain-C-1SOiN5.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode spoken Auto Drive handback | `9e64618`; Fly release `v694`; image `registry.fly.io/concrete-ops-2:deployment-01KTBPJY88XB7VXN5E9AKASNWA` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-105119Z.json` plus `uploads-20260605-105119Z` and `uploads-20260605-105119Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v693`, image `registry.fly.io/concrete-ops-2:deployment-01KTBMVMDCEGJK1W92QYYM1D7B`; both production `/api/ready` endpoints returned ready/database OK; `https://concrete-ops-2.fly.dev/api/health` returned healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 88ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-BnVgjuXG.js`, `app-domain-BjfMpLi0.js`, and `app-domain-C-1SOiN5.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, `/api/apex-os/voice/transcribe`, and `/api/apex-os/autonomy-runs/does-not-exist/advance-private` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode run memory review gate | `e4ed3fb`; Fly release `v692`; image `registry.fly.io/concrete-ops-2:deployment-01KTBJ7B3V7NEND74MSQV4DKYC` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-093502Z.json` plus `uploads-20260605-093502Z` and `uploads-20260605-093502Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; Fly SSH returned the known Windows handle warning after successful backup output and artifacts were separately confirmed; rollback target `v691`, image `registry.fly.io/concrete-ops-2:deployment-01KTBG1JFEGYWDSZ4DS57C9MX1`; both production `/api/ready` endpoints returned ready and readiness timing confirmed database OK; both `/api/health` endpoints healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 78ms for `/api/health` and 31ms for `/api/ready`; `/apex-control-room` served `index-BgCDqGyO.js`, `app-domain-0-NLc_yX.js`, and `app-domain-C-1SOiN5.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode private Work Loop | `c9e6e55`; Fly release `v691`; image `registry.fly.io/concrete-ops-2:deployment-01KTBG1JFEGYWDSZ4DS57C9MX1` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-085621Z.json` plus `uploads-20260605-085621Z` and `uploads-20260605-085621Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v690`, image `registry.fly.io/concrete-ops-2:deployment-01KTBD4HTM3F73QDS1ME5B3XCB`; both production `/api/ready` endpoints returned ready/database OK; `https://concrete-ops-2.fly.dev/api/health` returned healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing passed; `/apex-control-room` served `index-CLlm0_mF.js`, `app-domain-D1Nh_kX0.js`, and `app-domain-C-1SOiN5.css` on both production hosts; unauthenticated Apex OS build-awareness, memory, autonomy-runs, ask, and voice endpoints returned 401; setup status complete/demo off/demo user absent/public signup disabled. Production auth login smoke was not run because this release used skip-auth production smoke only. |
| 2026-06-05 | Apex Live Operator Mode voice health/recovery | `a1ec451`; Fly release `v690`; image `registry.fly.io/concrete-ops-2:deployment-01KTBD4HTM3F73QDS1ME5B3XCB` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy production backup `postgres-app-data-20260605-080556Z.json` plus `uploads-20260605-080556Z` and `uploads-20260605-080556Z.manifest.json` with 5 uploaded files; Fly SSH returned a Windows handle warning after successful backup output, and the artifacts were separately confirmed; machine `148e06e2b53d68` was woken before backup because it was stopped; rollback target `v689`, image `registry.fly.io/concrete-ops-2:deployment-01KTBB3GR36CKMBJ7BGPGRN7VM`; both `/api/ready` endpoints database OK; production `/api/health` healthy/200; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 95ms for `/api/health` and 32ms for `/api/ready`; `/apex-control-room` served `index-nOP-WsXy.js`, `app-domain-JpazMc3I.js`, and `app-domain-DBUdle4j.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode mobile/tablet body dock | `ac7b9db`; Fly release `v689`; image `registry.fly.io/concrete-ops-2:deployment-01KTBB3GR36CKMBJ7BGPGRN7VM` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-073029Z.json` plus `uploads-20260605-073029Z` and `uploads-20260605-073029Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v688`, image `registry.fly.io/concrete-ops-2:deployment-01KTB8EG8KQ79SHJZVEWX00VTM`; both `/api/ready` endpoints database OK; production `/api/health` healthy/200 through direct and hosted smoke checks; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 92ms for `/api/health` and 28ms for `/api/ready`; `/apex-control-room` served `index-DuTIRRH9.js`, `app-domain-D2xqh8et.js`, and `app-domain-Ctb5rfiB.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode spotlight dock | `09bdc88`; Fly release `v688`; image `registry.fly.io/concrete-ops-2:deployment-01KTB8EG8KQ79SHJZVEWX00VTM` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-064421Z.json` plus `uploads-20260605-064421Z` and `uploads-20260605-064421Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v687`, image `registry.fly.io/concrete-ops-2:deployment-01KTB74FFX3PYVRYA531T7BSW1`; both `/api/ready` endpoints database OK; both `/api/health` endpoints healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 86ms for `/api/health` and 33ms for `/api/ready`; `/apex-control-room` served `index-CbxfbknD.js`, `app-domain-CvhvviHz.js`, and `app-domain-D7AHsL2f.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode operator handback | `c59a795`; Fly release `v687`; image `registry.fly.io/concrete-ops-2:deployment-01KTB74FFX3PYVRYA531T7BSW1` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-062126Z.json` plus `uploads-20260605-062126Z` and `uploads-20260605-062126Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v686`, image `registry.fly.io/concrete-ops-2:deployment-01KTB5PC7E10A1QC586VDMHJMZ`; both `/api/ready` endpoints database OK; both `/api/health` endpoints healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 160ms for `/api/health` and 32ms for `/api/ready`; `/apex-control-room` served `index-BgItanF1.js`, `app-domain-DbE3BbZp.js`, and `app-domain-B_qRaull.css` on both production hosts; unauthenticated GET `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs` plus POST `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Now operator-state | `95ca7dd`; Fly release `v686`; image `registry.fly.io/concrete-ops-2:deployment-01KTB5PC7E10A1QC586VDMHJMZ` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-055614Z.json` plus `uploads-20260605-055614Z` and `uploads-20260605-055614Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v685`, image `registry.fly.io/concrete-ops-2:deployment-01KTB3TSQF9W8YF9PX1BD99H0W`; both `/api/ready` endpoints database OK; both `/api/health` endpoints healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 160ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-7bL1aaKZ.js`, `app-domain-Cra4UZoV.js`, and `app-domain-B_qRaull.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode compact body-first console | `51f6a14`; Fly release `v685`; image `registry.fly.io/concrete-ops-2:deployment-01KTB3TSQF9W8YF9PX1BD99H0W` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-052342Z.json` plus `uploads-20260605-052342Z` and `uploads-20260605-052342Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v684`, image `registry.fly.io/concrete-ops-2:deployment-01KTB2HXE9XZE604ECPJ37SPZR`; both `/api/ready` endpoints database OK; both `/api/health` endpoints healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 141ms for `/api/health` and 31ms for `/api/ready`; `/apex-control-room` served `index-MbqGBv6y.js`, `app-domain-w_HPSTv6.js`, and `app-domain-B_qRaull.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode memory-continuity prompt | `139778e`; Fly release `v684`; image `registry.fly.io/concrete-ops-2:deployment-01KTB2HXE9XZE604ECPJ37SPZR` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-050055Z.json` plus `uploads-20260605-050055Z` and `uploads-20260605-050055Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v683`, image `registry.fly.io/concrete-ops-2:deployment-01KTB1C2YD53NETYKNK49SV7K5`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 118ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-LAc-8heU.js`, `app-domain-CYCSG5eg.js`, and `app-domain-BxWZlvt3.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled on both production hosts. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode trusted live-run memory context | `fa6eb95`; Fly release `v683`; image `registry.fly.io/concrete-ops-2:deployment-01KTB1C2YD53NETYKNK49SV7K5` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-044043Z.json` plus `uploads-20260605-044043Z` and `uploads-20260605-044043Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v682`, image `registry.fly.io/concrete-ops-2:deployment-01KTAZDSTGS409E6HWGZGVW8RW`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 95ms for `/api/health` and 31ms for `/api/ready`; `/apex-control-room` served `index-DNLxKy9h.js`, `app-domain-BMWfjrT9.js`, and `app-domain-BxWZlvt3.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode proactive run-memory history | `38deb50`; Fly release `v682`; image `registry.fly.io/concrete-ops-2:deployment-01KTAZDSTGS409E6HWGZGVW8RW` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-040705Z.json` plus `uploads-20260605-040705Z` and `uploads-20260605-040705Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v681`, image `registry.fly.io/concrete-ops-2:deployment-01KTAX244HFNRNGQ0209G1AS63`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 95ms for `/api/health` and 31ms for `/api/ready`; `/apex-control-room` served `index-CCZ_HAkk.js`, `app-domain-Dab17VcV.js`, and `app-domain-BxWZlvt3.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode proactive check-in | `f5c64cd`; Fly release `v681`; image `registry.fly.io/concrete-ops-2:deployment-01KTAX244HFNRNGQ0209G1AS63` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-032516Z.json` plus `uploads-20260605-032516Z` and `uploads-20260605-032516Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v680`, image `registry.fly.io/concrete-ops-2:deployment-01KTAV6PNHCW5Z418VR32GJ1DV`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 70ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-DrVhL3Pd.js`, `app-domain-Bj4zPQF-.js`, and `app-domain-DDm4w_L0.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode live session heartbeat | `c73f8b2`; Fly release `v680`; image `registry.fly.io/concrete-ops-2:deployment-01KTAV6PNHCW5Z418VR32GJ1DV` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-025310Z.json` plus `uploads-20260605-025310Z` and `uploads-20260605-025310Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v679`, image `registry.fly.io/concrete-ops-2:deployment-01KTAS70215CH5HH4PABF92VPP`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 166ms for `/api/health` and 35ms for `/api/ready`; `/apex-control-room` served `index-CYZdpZgf.js`, `app-domain-UINyTgme.js`, and `app-domain-BwwrLrxU.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode immersive body-first desktop | `e9351f6`; Fly release `v679`; image `registry.fly.io/concrete-ops-2:deployment-01KTAS70215CH5HH4PABF92VPP` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-021827Z.json` plus `uploads-20260605-021827Z` and `uploads-20260605-021827Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v678`, image `registry.fly.io/concrete-ops-2:deployment-01KTAQS0ENB4YWZ6QF4BG7QVZJ`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 122ms for `/api/health` and 40ms for `/api/ready`; `/apex-control-room` served `index-BNdEmzH6.js`, `app-domain-DCQlUqcY.js`, and `app-domain-BwwrLrxU.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode natural next-turn follow-up | `5b48d5e`; Fly release `v678`; image `registry.fly.io/concrete-ops-2:deployment-01KTAQS0ENB4YWZ6QF4BG7QVZJ` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-015319Z.json` plus `uploads-20260605-015319Z` and `uploads-20260605-015319Z.manifest.json` with 5 uploaded files; rollback target `v677`, image `registry.fly.io/concrete-ops-2:deployment-01KTAPJYV1XX8STS1MQ67YG6EW`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 89ms for `/api/health` and 32ms for `/api/ready`; `/apex-control-room` served `index-CuEiq_fX.js`, `app-domain-VOh9wFYm.js`, and `app-domain-GJ2osZSJ.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode proactive operator judgment | `859f4c8`; Fly release `v677`; image `registry.fly.io/concrete-ops-2:deployment-01KTAPJYV1XX8STS1MQ67YG6EW` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-013225Z.json` plus `uploads-20260605-013225Z` and `uploads-20260605-013225Z.manifest.json` with 5 uploaded files; machine `148e06e2b53d68` was started before backup because it was stopped; rollback target `v676`, image `registry.fly.io/concrete-ops-2:deployment-01KTAM83BT2CVC9EEZNGYR64J7`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 89ms for `/api/health` and 31ms for `/api/ready`; `/apex-control-room` served `index-BHtamAyy.js`, `app-domain-Bt8NyB_7.js`, and `app-domain-BIcJPKOy.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode natural-command autopilot | `74e855c`; Fly release `v676`; image `registry.fly.io/concrete-ops-2:deployment-01KTAM83BT2CVC9EEZNGYR64J7` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-005119Z.json` plus `uploads-20260605-005119Z` and `uploads-20260605-005119Z.manifest.json` with 5 uploaded files; rollback target `v675`, image `registry.fly.io/concrete-ops-2:deployment-01KTAJ71FZ3FVKZGR4Y95EZZ5X`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 65ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-ByAgULNz.js`, `app-domain-FitZSPEd.js`, and `app-domain-DOBJTOUH.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
| 2026-06-05 | Apex Live Operator Mode run outcome memory | `13195e7`; Fly release `v675`; image `registry.fly.io/concrete-ops-2:deployment-01KTAJ71FZ3FVKZGR4Y95EZZ5X` | Production Fly app `concrete-ops-2`; `https://app.apexhq.online/` and `https://concrete-ops-2.fly.dev/` | Predeploy backup `postgres-app-data-20260605-001510Z.json` plus `uploads-20260605-001510Z` and `uploads-20260605-001510Z.manifest.json` with 5 uploaded files; rollback target `v674`, image `registry.fly.io/concrete-ops-2:deployment-01KTAFVP0VGJ4MHF8Q73CDA406`; both `/api/ready` endpoints database OK; `https://concrete-ops-2.fly.dev/api/health` healthy; Fly machine/checks passing on `148e06e2b53d68`; hosted skip-auth health/routes smoke passed on `https://app.apexhq.online/`; readiness timing averaged 273ms for `/api/health` and 30ms for `/api/ready`; `/apex-control-room` served `index-BS35XYIo.js`, `app-domain-CH2EbW5y.js`, and `app-domain-DOBJTOUH.css` on both production hosts; unauthenticated `/api/apex-os/build-awareness`, `/api/apex-os/memory`, `/api/apex-os/autonomy-runs`, `/api/apex-os/ask`, `/api/apex-os/voice/speech`, and `/api/apex-os/voice/transcribe` returned 401; setup status complete/demo off/demo user absent/public signup disabled/public estimate requests enabled. Production auth login smoke was not run because no dedicated production smoke env was used and login creates session/audit side effects. |
...
Use `docs/APEX_HQ_TOOL_COMPLETION_BLUEPRINT.md` as historical phase evidence, not an instruction to restart completed phases. The next phase is post-launch route/workflow QA refresh and cleanup triage. Build work should stay narrow and evidence-driven unless John explicitly approves a new roadmap phase. Live payroll processing, live billing/payment collection, public customer portal access, public self-serve signup, and risky external provider actions remain out of scope.
