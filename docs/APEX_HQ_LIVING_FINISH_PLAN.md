...
- The next recommended Apex OS step is to clear the production-auth-smoke/deploy gate for the read-only Control Room package, then connect Ask Apex to approved durable memory and provider-backed answers in a separate local slice with provider secrets and production use still isolated behind explicit approval packets.
