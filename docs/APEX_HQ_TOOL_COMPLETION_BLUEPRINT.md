...
| Takeoff Studio | Plan/PDF measurement and quantity backup for estimates. | Finished through Phase 20 review-first checkpoint; included in current production code ancestry | Owner/admin or estimator can measure plans, review quantities, connect quantity backup to blank-priced estimate line drafts, choose customer-safe proposal proof, prepare GC/internal takeoff summaries, use review-first assistant suggestions, carry explicitly approved field-safe quantity context into foreman handoff, export/import review-safe takeoff packages, view selected plan sheets from safe preview URLs, apply reviewed sheet calibration to measurement rows, draw area/length/count/volume draft measurements, snap draft geometry to endpoints/segments/intersections/angle increments, pin reviewed plan markup notes/RFIs/scope/risk comments, use local review-first Plan Assist over reviewed plan text/OCR notes, add draft-only auto-measure and trade-pack suggestions, register reviewed plan file/text sources, and review pilot hardening gates without exposing office data to field users. | Apex Takeoff Studio Phases 1-20 | Provider-backed OCR/vision, real image/PDF auto-measure, production telemetry, durable audit trails, deeper trade libraries, and richer collaboration later. | Freeze after Phase 20 review-first validation; future work only bugs, provider/PDF parsing hookups, pilot-proven workflow gaps, or approved versioned upgrades. |
