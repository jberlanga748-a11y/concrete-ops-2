...
| Takeoff Studio | Plan/PDF measurement and quantity backup for estimates. | Finished locally through Phase 8; snapping, AI plan assist, auto-measure beta, and hardening active next | Owner/admin or estimator can measure plans, review quantities, connect quantity backup to blank-priced estimate line drafts, choose customer-safe proposal proof, prepare GC/internal takeoff summaries, use review-first assistant suggestions, carry explicitly approved field-safe quantity context into foreman handoff, export/import review-safe takeoff packages, view selected plan sheets from safe preview URLs, apply reviewed sheet calibration to measurement rows, and draw area/length/count/volume draft measurements without exposing office data to field users. | Apex Takeoff Studio Phases 1-8 | Snapping, provider-backed AI plan assistance, provider/file parsing upgrades, OCR/PDF parsing, auto-measure beta, and richer collaboration later. | Freeze Phase 8 after validation; future work continues only as approved Takeoff Studio Phases 9-13, bugs, provider/PDF parsing hookups, pilot-proven workflow gaps, or approved versioned upgrades. |
