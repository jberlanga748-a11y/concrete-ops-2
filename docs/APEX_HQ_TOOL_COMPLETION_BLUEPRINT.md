...
| Takeoff Studio | Plan/PDF measurement and quantity backup for estimates. | Finished through Phase 5; versioned provider/PDF parsing later | Owner/admin or estimator can measure plans, review quantities, connect quantity backup to blank-priced estimate line drafts, choose customer-safe proposal proof, prepare GC/internal takeoff summaries, use review-first assistant suggestions, carry explicitly approved field-safe quantity context into foreman handoff, and export/import review-safe takeoff packages without exposing office data to field users. | Apex Takeoff Studio Phases 1-5 | Provider-backed AI plan assistance, provider/file parsing upgrades, OCR/PDF parsing, and richer collaboration later. | Freeze after Phase 5; future work only bugs, provider/PDF parsing hookups, pilot-proven workflow gaps, or approved versioned upgrades. |
