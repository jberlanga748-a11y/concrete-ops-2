...
| Takeoff Studio | Plan/PDF measurement and quantity backup for estimates. | Finished locally through Phase 13; provider/PDF vision upgrades later | Owner/admin or estimator can measure plans, review quantities, connect quantity backup to blank-priced estimate line drafts, choose customer-safe proposal proof, prepare GC/internal takeoff summaries, use review-first assistant suggestions, carry explicitly approved field-safe quantity context into foreman handoff, export/import review-safe takeoff packages, view selected plan sheets from safe preview URLs, apply reviewed sheet calibration to measurement rows, draw area/length/count/volume draft measurements, snap draft geometry to endpoints/segments/intersections/angle increments, pin reviewed plan markup notes/RFIs/scope/risk comments, use local review-first Plan Assist over reviewed plan text/OCR notes, add Auto-Measure Beta draft suggestions, and review hardening readiness without exposing office data to field users. | Apex Takeoff Studio Phases 1-13 | Provider-backed AI plan assistance, provider/file parsing upgrades, OCR/PDF parsing, real image/PDF auto-measure, large-PDF telemetry, and richer collaboration later. | Freeze after Phase 13 validation; future work only bugs, provider/PDF parsing hookups, pilot-proven workflow gaps, or approved versioned upgrades. |
